...
| `family_variant_derivation.representative_library_recipe.pretrained_disable_fields` | array<string> | Mandatory | Mandatory pretrained disable fields. Names every pinned-constructor keyword that would otherwise load pretrained weights or a checkpoint. This is checked, not merely recorded: R1_LIBRARY REFUSES an empty array, every name here must also be a key of kwargs carrying a disabling value (null, false, empty string, 'none', or 'random'), names must not repeat, and each name must be a real keyword parameter of the pinned constructor -- a name absent from that signature is refused when the recipe is loaded. So an empty array cannot be filled by inventing a field. If the pinned constructor genuinely exposes no pretrained, weights, or checkpoint keyword, R1_LIBRARY's precondition cannot be met for it; report that as a blocked prerequisite rather than emitting a recipe that will be refused as malformed. |
...
| `implementation.code_path` | string \| null | Mandatory | Mandatory code path. Model-root-relative entry module for a staged rung; null for R1_LIBRARY, which refuses staged code and must use library_recipe instead. Staged code is checked by AST before acceptance and must be FULLY TYPED: every function and method defined in this module, and in every member of its recursive model-local import closure listed in code_manifest, carries an annotation on every parameter other than self/cls -- including *args and **kwargs -- and a return annotation. One unannotated definition anywhere in that closure refuses the whole proposal. Verbatim-vendored upstream files are part of the closure and are checked identically; unannotated upstream source is a blocking condition to report, never a reason to modify the vendored bytes. |
| `implementation.code_sha256` | string \| null | Mandatory | Mandatory code sha256. |
| `implementation.code_manifest` | array<object> | Optional | Closed recursive model-local Python import manifest. Every member listed here is subject to the same full-annotation AST check as code_path, vendored upstream files included. |
