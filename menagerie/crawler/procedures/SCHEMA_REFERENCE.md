...
| `source_resolution.search_report.links_checked` | array<string \| object> | Mandatory | Mandatory exact content URLs checked. For R2/R4 every string entry must bind to fetched, hash-inventoried CAS bytes; an HTTP or otherwise policy-refused link may instead be typed as {url, disposition:"unfetchable-by-policy"} and is accepted only with a matching broker receipt. |
| `source_resolution.search_report.links_checked[].url` | string | Mandatory | Exact URL checked. |
| `source_resolution.search_report.links_checked[].disposition` | const `unfetchable-by-policy` | Mandatory | Typed policy refusal; valid only when the broker receipt for this URL records unfetchable-by-policy or redirect-refused. |
