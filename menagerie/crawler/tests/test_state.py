...
def test_sqlite_rebuild_materializes_current_revision(tmp_path: Path) -> None:
    ...
    context = make_authority_context(("m_example",))
    with CanonicalReducer(paths, context) as reducer:
        reducer.append_attempt(failed_attempt)
        persisted = reducer.append_model(reducer.prepare_model(first)).record
        ...
        reducer.append_model(reducer.prepare_model(second))
    database = tmp_path / "state.sqlite3"
    summary = rebuild_state(database, ["m_example"], paths, context=context)
