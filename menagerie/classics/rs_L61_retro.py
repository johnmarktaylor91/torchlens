# SOURCE: vendored from lucidrains/RETRO-pytorch @ main
#
# https://github.com/lucidrains/RETRO-pytorch
# https://raw.githubusercontent.com/lucidrains/RETRO-pytorch/main/retro_pytorch/retro_pytorch.py
#
# Borgeaud et al. 2022 "Improving Language Models by Retrieving from Trillions
# of Tokens" (DeepMind RETRO). This is lucidrains' widely-used community
# implementation (pip package `retro-pytorch`), a high-quality reproduction of
# the chunked cross-attention (CCA) retrieval-augmented transformer. The model
# class below (`RETRO` + its `Encoder`/`Decoder`/`ChunkedCrossAttention`
# submodules) is copied verbatim from `retro_pytorch/retro_pytorch.py` with a
# single minimal fix: the upstream file imports `BERT_VOCAB_SIZE` from
# `retro_pytorch.retrieval`, a module whose *own* imports (`faiss`,
# `autofaiss`) are retrieval-index-building infrastructure unrelated to the
# nn.Module architecture and are not installed base libs. `BERT_VOCAB_SIZE` is
# just the integer constant `28996` (bert-base-cased vocab size) defined at
# `retro_pytorch/retrieval.py:20`; it is inlined here rather than importing
# the whole retrieval module, since the constant's value -- not any code
# behavior -- is what it contributes to this file. No other line of the
...
# BERT_VOCAB_SIZE inlined from retro_pytorch/retrieval.py:20 (bert-base-cased
# vocab size) -- avoids importing retro_pytorch.retrieval, which pulls in
# faiss/autofaiss (retrieval-index building, not part of the nn.Module).
...
def apply_rotary_pos_emb(t, freqs):
    seq_len, rot_dim = t.shape[-2], freqs.shape[-1]  # noqa: F841 (unused seq_len is vendored as-is from upstream)
...
class Decoder(nn.Module):
    ...
    def forward(
        self, x, *, encoder=None, encoder_retrieved_mask=None, context_mask=None, retrieved=None
    ):
        ...
        for attn, cross_attn, ff in self.layers:
            ...
            if exists(cross_attn) and exists(retrieved):
                if not retrieved_encoded:
                    ...
                    retrieved = encoder(
                        retrieved, mask=encoder_retrieved_mask, chunked_seq=seq_as_context
                    )
                    retrieved = rearrange(
                        retrieved, "(b k r) n d -> b k r n d", k=num_chunks, r=num_neighbors
                    )
                    retrieved_encoded = True

                x = cross_attn(
                    x, context=retrieved, context_mask=context_mask, pos_emb=cross_attn_pos_emb
                )
    ...
    def forward(self, seq, retrieved=None, return_loss=False):
        ...
        n, num_chunks, num_neighbors, chunk_size, retrieved_shape, device = (
            ...
            *retrieved.shape[-3:],
            retrieved.shape,
            seq.device,
        )  # noqa: F841 (unused num_neighbors is vendored as-is from upstream)
        ...
        assert num_chunks == num_seq_chunks, (
        ...
        seq_index = num_seq_chunks * self.chunk_size  # noqa: F841 (unused seq_index is vendored as-is from upstream)
        ...
        if exists(mask):
            assert mask.shape == retrieved_shape, (
...
def example_input_retro():
    # sequence made of 2 chunks of chunk_size=8 -> seq_len=16
    seq = torch.randint(1, 200, (1, 16))
    # 2 chunks, 1 retrieved neighbor per chunk, chunk_size=8 retrieved tokens
    retrieved = torch.randint(1, 200, (1, 2, 1, 8))
    return (seq, retrieved)


MENAGERIE_ZOO = "vendored-pytorch"
MENAGERIE_ENTRIES = [
    ("RETRO", "build_retro", "example_input_retro", 2022, "vendored"),
]
