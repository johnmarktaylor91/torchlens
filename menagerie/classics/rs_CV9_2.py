# SOURCE: vendored from yan-hao-tian/ConTNet @ master (ConTNet.py)
...
class MHSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        qkv = (
            self.qkv(x).reshape(B, N, 3, H, C // H).permute(2, 0, 3, 1, 4)
        )  # x: (3, B, H, N, C//H)
        ...
        if self.relative:
            ...
            ].view(self.patch_size**2, self.patch_size**2, -1)
...
class STE(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attn = MHSA(
            planes, head_num, dropout, patch_size, qkv_bias=qkv_bias, relative=relative
        )
    ...
    def forward(self, x):
        ...
        if not self.relative:
            ...
            x = torch.cat(
                (x_h + self.pe[0], x_w + self.pe[1]), dim=3
...
def build_contnet():
    """Build a trace-sized ConTNet-Ti."""
    return build_model(
        arch="ConT-Ti", use_avgdown=True, relative=True, qkv_bias=True, pre_norm=True
    )


def example_input_contnet():
    """Return an example ConTNet input image."""
    return torch.randn(1, 3, 224, 224)


MENAGERIE_ENTRIES = [
    ("ConTNet", "build_contnet", "example_input_contnet", 2021, "CV9"),
]
