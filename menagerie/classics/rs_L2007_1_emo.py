# SOURCE: vendored from zhangzjn/EMO @ 27d9b40fea9c2a12a2fe7acc51032184852a99d8
# (model/emo.py + model/basic_modules.py) -- the real iRMB (inverted Residual Mobile Block)
# and EMO classification model classes are preserved verbatim; only the timm import paths
# are updated from the pre-1.0 `timm.models.layers.activations` / `timm.models.layers`
# deprecated shim locations (which no longer exist in modern timm) to the current
# `timm.layers.activations` / `timm.layers` / `timm.layers.squeeze_excite` module layout.
# The `model.basic_modules` / `model.__init__` package-relative imports are inlined into
# this single file since the original multi-file package layout is not vendored. No
# architectural change.
"""EMO: Rethinking Mobile Block for Efficient Attention-based Models.

Zhang, Jiangning, et al. "Rethinking Mobile Block for Efficient Attention-based Models."
ICCV 2023. Introduces the iRMB (inverted Residual Mobile Block), a unified building block
that fuses MobileNet-style inverted-residual depthwise convolution with lightweight
windowed multi-head self-attention, used to build efficient hybrid CNN/Transformer
classification backbones (EMO-1M/2M/5M/6M).
"""
...
from timm.layers.squeeze_excite import SqueezeExcite as SE

MENAGERIE_ZOO = "vendored-pytorch"

__all__ = ["EMO", "EMO_1M", "EMO_2M", "EMO_5M", "EMO_6M"]

inplace = True


# ========== basic_modules.py (verbatim, package-relative imports inlined) ==========
...
class MSPatchEmb(nn.Module):
    def __init__(
        ...
        for i in range(len(dilations)):
            ...
            self.convs.append(
                nn.Sequential(
                    nn.Conv2d(
                        dim_in, emb_dim, kernel_size, stride, padding, dilations[i], groups=c_group
                    ),
    ...
    def forward(self, x):
        if self.dilation_num == 1:
            ...
            x = torch.cat(
                [self.convs[i](x).unsqueeze(dim=-1) for i in range(self.dilation_num)], dim=-1
            )
...
class iRMB(nn.Module):
    def __init__(
        ...
        self.proj = ConvNormAct(
            dim_mid, dim_out, kernel_size=1, norm_layer="none", act_layer="none", inplace=inplace
        )
    ...
    def forward(self, x):
        ...
        if self.attn_s:
            ...
            if self.attn_pre:
                x = rearrange(
                    x, "b (heads dim_head) h w -> b heads (h w) dim_head", heads=self.num_head
                ).contiguous()
                x_spa = attn_spa @ x
                x_spa = rearrange(
                    x_spa,
                ...
                v = self.v(x)
                v = rearrange(
                    v, "b (heads dim_head) h w -> b heads (h w) dim_head", heads=self.num_head
                ).contiguous()
                x_spa = attn_spa @ v
                x_spa = rearrange(
                    x_spa,
    ...
    def __init__(
        ...
        if pre_dim > 0:
            self.pre_head = nn.Sequential(
                nn.Linear(embed_dims[-1], pre_dim), get_act(act_layers[-1])(inplace=inplace)
            )
...
def EMO_6M(pretrained=False, **kwargs):
    model = EMO(
        depths=[3, 3, 9, 3],
        stem_dim=24,
        embed_dims=[48, 72, 160, 320],
        exp_ratios=[2.0, 3.0, 4.0, 5.0],
        norm_layers=["bn_2d", "bn_2d", "ln_2d", "ln_2d"],
        act_layers=["silu", "silu", "gelu", "gelu"],
        dw_kss=[3, 3, 5, 5],
        dim_heads=[16, 24, 20, 32],
        window_sizes=[7, 7, 7, 7],
        attn_ss=[False, False, True, True],
        qkv_bias=True,
        attn_drop=0.0,
...
def example_input_emo():
    return torch.randn(1, 3, 64, 64)


MENAGERIE_ENTRIES = [
    ("EMO-1M (iRMB mobile hybrid CNN-Transformer)", EMO_1M, example_input_emo, 2023, "vendored"),
]
