# SOURCE: vendored from xinjli/allosaurus @ master (allosaurus/am/allosaurus_torch.py)
...
class AllosaurusConfig:
    """Tiny Allosaurus configuration for TorchLens tracing."""

    def __init__(self) -> None:
        """Initialize the config attributes expected by the vendored model."""
        self.hidden_size = 8
        self.layer_size = 1
        self.proj_size = 0
        self.feat_size = 12
        self.lang_size_dict = {"eng": 16}
        self.phone_size = 16
...
class AllosaurusTorchModel(nn.Module):
    def __init__(self, config):
        ...
        self.blstm_layer = nn.LSTM(
            self.feat_size, self.hidden_size, num_layers=self.layer_size, bidirectional=True
        )
    ...
    def add_args(parser):
        parser.add_argument(
            ...
            type=int,
            default=-1,
            help="input size in the blstm model. if -1, then it is determined automatically by loader",
        )
        parser.add_argument(
            "--hidden_size", type=int, default=320, help="hidden size in the blstm model"
        )
        parser.add_argument(
            "--lang_size",
            type=int,
            default=-1,
            help="output size in the blstm model, if -1, then it is determined automatically by loader",
        )
        parser.add_argument("--proj_size", type=int, default=0, help="projection")
        parser.add_argument(
            "--layer_size", type=int, default=5, help="layer size in the blstm model"
        )
        ...
        parser.add_argument(
            "--debug_model", type=str, default=False, help="print tensor info for debugging"
        )
    ...
    def forward(self, input_tensor, input_lengths, return_lstm=False, return_both=False, meta=None):
        ...
        output_tensor, _ = nn.utils.rnn.pad_packed_sequence(
            hidden_pack_sequence, total_length=total_length
        )
...
def build_allosaurus() -> AllosaurusTorchModel:
    """Build a tiny Allosaurus BLSTM recognizer for TorchLens tracing."""
    model = AllosaurusTorchModel(AllosaurusConfig())
    model.eval()
    return model


def example_input_allosaurus() -> tuple[torch.Tensor, torch.Tensor]:
    """Return batched acoustic features and sequence lengths."""
...
MENAGERIE_ENTRIES = [("Allosaurus", "build_allosaurus", "example_input_allosaurus", 2020, "CV1")]
