# SOURCE: vendored from https://github.com/polarisZhao/PFLD-pytorch @ b00750c37deaf5d133933d080d4b7a6421826812
# (models/pfld.py) -- PFLD: A Practical Facial Landmark Detector (arXiv:1902.10859)
"""PFLD facial landmark detector, vendored verbatim from the official PFLD-pytorch repo.

Only minimal changes were made relative to the upstream file:
  - none to the architecture; the PFLDInference / AuxiliaryNet classes and InvertedResidual
    block below are byte-for-byte the same forward/module structure as models/pfld.py.
"""
...
def build_pfld():
    """Real PFLDInference landmark backbone (excludes the AuxiliaryNet pose head,
    which is only used during training to regress head-pose Euler angles)."""
    return PFLDInference()


def example_input_pfld():
    return torch.randn(1, 3, 112, 112)


MENAGERIE_ENTRIES = [
    ("PFLD", "build_pfld", "example_input_pfld", 2019, "SOURCE_AVAILABLE"),
]
