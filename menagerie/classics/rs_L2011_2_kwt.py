# SOURCE: vendored from mashrurmorshed/Torch-KWT @ main (models/kwt.py), a PyTorch re-implementation
# of the official ARM Software Keyword Transformer (ARM-software/keyword-transformer, Interspeech 2021),
# whose own release ships only the TensorFlow original. Imports/relative-paths adjusted minimally for
# standalone staging; architecture untouched (plain ViT-style transformer over MFCC + SpecAug patches).
...
def build_kwt():
    """Tiny Keyword Transformer (KWT-1 config, shrunk depth/dim for a fast trace)."""
    torch.manual_seed(0)
    return KWT(
        input_res=[40, 98],
        patch_res=[40, 1],
...
MENAGERIE_ENTRIES = [
    ("Keyword-Transformer", build_kwt, example_input_kwt, 2021, "vendored-pytorch"),
]
