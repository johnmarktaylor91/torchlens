# SOURCE: vendored from https://github.com/lightvector/KataGo @ a339686e5eaa
#   (python/katago/train/modelconfigs.py: ModelConfig + named configs incl. b6c96;
#    python/katago/train/model_pytorch.py: Model + all block/head submodules)
#
# KataGo (Wu, 2019, "Accelerating Self-Play Learning in Go" + ongoing KataGo project). This is
# KataGo's own official PyTorch training-side network definition -- the exact module graph that
# gets exported to the C++/CUDA engine, not a third-party reimplementation. `modelconfigs.py`
# ships several named reference configs; "b6c96" is KataGo's own "Historic katago architecture"
# entry (6 residual blocks, 96 trunk channels) -- the smallest official named config, used here
# for a fast trace. Both source files are vendored verbatim and flattened into one module
# (KataGo's own package splits them across `katago/train/{modelconfigs,model_pytorch}.py`);
# the only edits are removing the relative import `from ..train import modelconfigs` and the
# now-redundant `modelconfigs.` prefix on references, since both files share this module's flat
# namespace instead of KataGo's package tree.
...
b40c768nbt = {
    ...
    "gamma_weight_decay_center_1": True,
    "block_kind": [
        [f"rconv{i}", "bottlenest2gpool" if i % 3 == 0 else "bottlenest2"] for i in range(1, 41)
    ],
    "p1_num_channels": 128,
    "g1_num_channels": 128,
    "v1_num_channels": 256,
...
b5c48h3tfr = {
    ...
    "block_kind": [
        item for i in range(1, 6) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffng"]]
    ],
    "p1_num_channels": 16,
    "g1_num_channels": 16,
    "v1_num_channels": 16,
...
b7c96h3tfrs = {
    ...
    "transformer_kv_heads": 3,
    "block_kind": [
        item for i in range(1, 8) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 32,
    "g1_num_channels": 32,
    "v1_num_channels": 32,
...
b8c96h3tfrs = {
    ...
    "transformer_kv_heads": 3,
    "block_kind": [
        item for i in range(1, 9) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 32,
    "g1_num_channels": 32,
    "v1_num_channels": 32,
...
b9c96h3tgabs = {
    ...
    "gab_mlp_hidden": 96,
    "block_kind": [
        item for i in range(1, 10) for item in [[f"attn{i}", "attngab"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 32,
    "g1_num_channels": 32,
    "v1_num_channels": 32,
...
b10c192h6tgabs = {
    ...
    "gab_mlp_hidden": 128,
    "block_kind": [
        item for i in range(1, 11) for item in [[f"attn{i}", "attngab"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 32,
    "g1_num_channels": 32,
    "v1_num_channels": 32,
...
b14c192h6tfrs = {
    ...
    "transformer_kv_heads": 6,
    "block_kind": [
        item for i in range(1, 15) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 32,
    "g1_num_channels": 32,
    "v1_num_channels": 32,
...
b16c256h8tfrs = {
    ...
    "transformer_kv_heads": 8,
    "block_kind": [
        item for i in range(1, 17) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 48,
    "g1_num_channels": 48,
    "v1_num_channels": 96,
...
b21c384h12tfrs = {
    ...
    "transformer_kv_heads": 12,
    "block_kind": [
        item for i in range(1, 22) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 64,
    "g1_num_channels": 64,
    "v1_num_channels": 128,
...
b30c512h16tflrs = {
    ...
    "learnable_rope": True,
    "block_kind": [
        item for i in range(1, 31) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 96,
    "g1_num_channels": 96,
    "v1_num_channels": 192,
...
b22c192h6tfrs = {
    ...
    "transformer_kv_heads": 6,
    "block_kind": [
        item for i in range(1, 23) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 48,
    "g1_num_channels": 48,
    "v1_num_channels": 96,
...
b45c384h12tflrs = {
    ...
    "learnable_rope": True,
    "block_kind": [
        item for i in range(1, 46) for item in [[f"attn{i}", "attnrope"], [f"ffn{i}", "ffnsg"]]
    ],
    "p1_num_channels": 96,
    "g1_num_channels": 96,
    "v1_num_channels": 192,
...
# ---- python/katago/train/model_pytorch.py (vendored verbatim below) ----
...
def debug_print_tensor(tensor):
    ...
    print(f"TOTAL {out.shape} {total1} {total2} {total3}")  # noqa: F821 -- vendored verbatim; pre-existing upstream bug in an unused debug helper (out undefined; never called by the model)
...
def compute_gain(activation):
    if activation == "relu" or activation == "hardswish":
        ...
        gain = math.sqrt(
            2.0
        )  # Theoretically should be sqrt(2.8108), kept sqrt(2.0) for compat reasons.
...
class RMSNormMask(torch.nn.Module):
    ...
    def __init__(self, c_in, config: ModelConfig, spatial: bool, cgroup_size: Optional[int]):
        ...
        if cgroup_size is not None:
            ...
            assert c_in % cgroup_size == 0, (
                f"c_in ({c_in}) must be divisible by cgroup_size ({cgroup_size})"
            )
...
class KataConvAndGPool(torch.nn.Module):
    ...
    def initialize(self, scale):
        ...
        if (
            ...
            init_weights(
                self.conv1g.weight, self.activation, scale=math.sqrt(scale) * math.sqrt(g_scale)
            )
            init_weights(
                self.linear_g.weight, self.activation, scale=math.sqrt(scale) * math.sqrt(g_scale)
            )
        else:
            init_weights(self.conv1r.weight, self.activation, scale=scale * r_scale)
    ...
    def forward(
        ...
        block_shared_data: Optional[Dict[str, Any]] = None,
    ):
        """
        Parameters:
        x: NCHW
...
class NormActConv(torch.nn.Module):
    def __init__(
        ...
        self.use_repvgg_init = (
            kernel_size > 1 and "use_repvgg_init" in config and config["use_repvgg_init"]
        )
...
class ResBlock(torch.nn.Module):
    ...
    def forward(
        ...
        out = self.normactconv1(
            out, mask=mask, mask_sum_hw=mask_sum_hw, mask_sum=mask_sum, extra_outputs=extra_outputs
        )
...
class DilationNestedBottleneckResBlock(torch.nn.Module):
    ...
    def forward(
        ...
        out = self.normactconvp(
            out, mask=mask, mask_sum_hw=None, mask_sum=None, extra_outputs=extra_outputs
        )
        ...
        assert tuple(out.shape) == (n, 3, 3, c, padded_h_div3, padded_w_div3), (
            f"{tuple(out.shape)=}"
        )
        ...
        for i in range(self.internal_length):
            out = out + self.blockstack[i](
                out, mask=mask_t, mask_sum_hw=None, mask_sum=None, extra_outputs=extra_outputs
            )

        # untranspose!
        out = out.reshape((n, 3, 3, c, padded_h_div3, padded_w_div3))
        out = out.permute(0, 3, 4, 1, 5, 2)
        assert tuple(out.shape) == (n, c, padded_h_div3, 3, padded_w_div3, 3), (
            f"{tuple(out.shape)=}"
        )
...
def compute_learnable_rope_cos_sin(s_x, s_y, freqs):
    ...
    angles = (
        s_x.unsqueeze(-1).unsqueeze(-1) * freqs[:, :, 0]
        + s_y.unsqueeze(-1).unsqueeze(-1) * freqs[:, :, 1]
    )
...
class GABTemplateMLP(torch.nn.Module):
    ...
    def __init__(
        self, gab_num_templates, gab_num_fourier_features, gab_mlp_hidden, pos_len, activation
    ):
        ...
        init_freqs = torch.exp(
            torch.linspace(math.log(1.0), math.log(1.0 / 50.0), gab_num_fourier_features)
        )
...
class TABModule(torch.nn.Module):
    ...
    def __init__(
        ...
        self.input_proj = torch.nn.Conv2d(
            trunk_channels, 2 * tab_num_freqs * tab_c_z, kernel_size=1, bias=False
        )
    ...
    def forward(self, x, mask):
        ...
        angles = self.rope_freqs[:, 0:1].unsqueeze(-1) * grid_x.unsqueeze(0) + self.rope_freqs[
            :, 1:2
        ].unsqueeze(-1) * grid_y.unsqueeze(0)  # (F, H, W)
...
class ComplexDepthwiseConv2d(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        conv1 = torch.nn.functional.conv2d(
            x_ri, k_ri, padding=padding, dilation=self.dilation, groups=2 * self.c
        )
        ...
        conv2 = torch.nn.functional.conv2d(
            x_ri, k_neg_ir, padding=padding, dilation=self.dilation, groups=2 * self.c
        )
...
class FrequencyMixingTABModule(torch.nn.Module):
    ...
    def __init__(
        ...
        self.query_proj = torch.nn.Conv2d(
            2 * tab_c_z, 2 * tab_c_z * tab_num_templates, kernel_size=1, bias=False
        )
    ...
    def forward(self, x, mask):
        ...
        angles = self.rope_freqs[:, 0:1].unsqueeze(-1) * grid_x.unsqueeze(0) + self.rope_freqs[
            :, 1:2
        ].unsqueeze(-1) * grid_y.unsqueeze(0)  # (c_z, H, W)
        ...
        cos_a_out = (
            cos_a.view(1, c_z, 1, 1, H, W).expand(N, c_z, 1, 1, H, W).reshape(N * c_z, 1, 1, H, W)
        )
        sin_a_out = (
            sin_a.view(1, c_z, 1, 1, H, W).expand(N, c_z, 1, 1, H, W).reshape(N * c_z, 1, 1, H, W)
        )
...
class TransformerAttentionBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_rope:
            assert self.q_head_dim % 4 == 0, "Query head dim must be divisible by 4 for 2D RoPE"
        ...
        if self.use_rope:
            if self.learnable_rope:
                ...
                assert self.rope_theta > pos_len * 2.0, (
                    f"theta={self.rope_theta} of RoPE may be too small for pos_len={pos_len}"
                )
        ...
        if self.use_gab or self.use_tab:
            ...
            self.gab_proj3 = torch.nn.Linear(
                gab_d2, self.num_heads * self.total_num_weights, bias=False
            )
    ...
    def forward(
        ...
        if self.use_rope:
            if self.learnable_rope:
                ...
                cos_k, sin_k = compute_learnable_rope_cos_sin(
                    s_x, s_y, self.rope_freqs
                )  # ([B,] S, H_kv, P)
                ...
                if self.n_rep > 1:
                    cos_q = (
                        cos_k.unsqueeze(-2)
                        .expand(*cos_k.shape[:-1], self.n_rep, cos_k.shape[-1])
                        .reshape(*cos_k.shape[:-2], self.num_heads, -1)
                    )
                    sin_q = (
                        sin_k.unsqueeze(-2)
                        .expand(*sin_k.shape[:-1], self.n_rep, sin_k.shape[-1])
        ...
        if self.n_rep > 1:
            k = k.unsqueeze(2).expand(
                batch_size, self.num_kv_heads, self.n_rep, seq_len, self.q_head_dim
            )
            k = k.reshape(batch_size, self.num_heads, seq_len, self.q_head_dim)
            v = v.unsqueeze(2).expand(
                batch_size, self.num_kv_heads, self.n_rep, seq_len, self.v_head_dim
            )
        ...
        if self.use_gab or self.use_tab:
            template_bias, extra_kq = self._compute_gab_bias(
                x_norm, mask, mask_sum_hw, block_shared_data
            )
        ...
        wants_attn_weights = (
            extra_outputs is not None and self.name + ".attn_weights" in extra_outputs.requested
        )
...
class PolicyHead(torch.nn.Module):
    ...
    def forward(
        ...
        return torch.cat(
            (outpolicy.view(outpolicy.shape[0], outpolicy.shape[1], -1), outpass.unsqueeze(-1)),
            dim=2,
        )
...
class ValueHead(torch.nn.Module):
    ...
    def initialize(self):
        ...
        init_weights(
            self.linear2.bias, self.activation, scale=bias_scale, fan_tensor=self.linear2.weight
        )
        ...
        init_weights(
            self.linear_s2.bias, self.activation, scale=1.0, fan_tensor=self.linear_s2.weight
        )
        init_weights(
            self.linear_s2off.weight, self.activation, scale=1.0, fan_tensor=self.linear_s2.weight
        )
        init_weights(
            self.linear_s2par.weight, self.activation, scale=1.0, fan_tensor=self.linear_s2.weight
        )
        ...
        init_weights(
            self.linear_smix.bias, "identity", scale=bias_scale, fan_tensor=self.linear_smix.weight
        )
...
class MetadataEncoder(torch.nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.meta_encoder_version = (
            1
            if "meta_encoder_version" not in config["metadata_encoder"]
            else config["metadata_encoder"]["meta_encoder_version"]
        )

        self.c_input = get_num_meta_encoder_input_features(self.meta_encoder_version)
    ...
    def initialize(self):
        ...
        with torch.no_grad():
            ...
            init_weights(
                self.linear1.bias, self.activation, scale=bias_scale, fan_tensor=self.linear1.weight
            )
...
class Model(torch.nn.Module):
    def __init__(self, config: ModelConfig, pos_len: int):
        ...
        for block_config in self.block_kind:
            ...
            if block_kind == "regular":
                ...
                self.blocks.append(
                    DilationNestedBottleneckResBlock(
                        name=block_name,
                        internal_length=2,
                        c_main=self.c_trunk,
                        c_mid=self.c_mid,
        ...
        if self.use_trunk_channel_gate:
            ...
            for k in range(num_blocks):
                self.trunk_channel_gate_logits.append(
                    torch.nn.Parameter(torch.zeros(1, self.c_trunk, 1, 1))
                )
        ...
        if self.use_trunk_residual_backout:
            ...
            self.backout_add_logit_embedding = torch.nn.Parameter(
                torch.zeros(1, self.c_trunk, 1, 1)
            )
            ...
            for k in range(num_blocks - 1):
                self.backout_add_logits.append(
                    torch.nn.Parameter(torch.zeros(1, self.c_trunk, 1, 1))
                )
            ...
            for k in range(num_blocks - 1):
                self.backout_use_logits.append(
                    torch.nn.Parameter(torch.full((1, self.c_trunk, 1, 1), -2.0))
                )
            ...
            self.backout_use_logit_final = torch.nn.Parameter(
                torch.full((1, self.c_trunk, 1, 1), -2.0)
            )
        ...
        if self.trunk_final_rmsnorm:
            ...
            self.norm_trunkfinal = RMSNormMask(
                self.c_trunk, self.config, spatial=spatial, cgroup_size=cgroup_size
            )
    ...
    def _trunk_residual_factors(self, block_idx):
        ...
        if self.use_trunk_channel_gate:
            ...
            w = ((block_idx + 2) / (block_idx + 1)) / (
                (1.0 / (block_idx + 1)) + torch.exp(-gate_logit)
            )
    ...
    def _run_block_with_backout(
        self,
        block,
        out,
        backout,
        block_idx,
        is_first_block_of_trunk,
        mask,
        mask_sum_hw,
        mask_sum,
        extra_outputs,
        ...
        if is_last_block_of_trunk:
            ...
            new_backout = (
                trunk_factor * backout + residual_factor * backout_residual_gate * residual
            )
        if extra_outputs is not None:
    ...
    def forward(
        ...
        if self.use_rw_registers:
            ...
            # mask: (B, 1, H, W)
            B, _, H, W = mask.shape
            col_idx = torch.arange(W, device=mask.device, dtype=mask.dtype).view(
                1, 1, 1, W
            )  # (1,1,1,W)
            row_idx = torch.arange(H, device=mask.device, dtype=mask.dtype).view(
                1, 1, H, 1
            )  # (1,1,H,1)
            inv_mask = 1.0 - mask
            col_min = (
                (col_idx * mask + W * inv_mask).amin(dim=(1, 2, 3), keepdim=False).unsqueeze(1)
            )  # (B, 1)
            col_max = (col_idx * mask).amax(dim=(1, 2, 3), keepdim=False).unsqueeze(1)  # (B, 1)
            row_min = (
                (row_idx * mask + H * inv_mask).amin(dim=(1, 2, 3), keepdim=False).unsqueeze(1)
            )  # (B, 1)
            ...
            rw_pos_x = center_x + 2.0 * radius_x * torch.tanh(
                self.rw_register_rel_pos_x.unsqueeze(0)
            )  # (B, k)
            rw_pos_y = center_y + 2.0 * radius_y * torch.tanh(
                self.rw_register_rel_pos_y.unsqueeze(0)
            )  # (B, k)
            ...
            if self.inline_registers:
                ...
                reg_mask = torch.ones(
                    B, 1, 1, self.num_rw_registers, device=mask.device, dtype=mask.dtype
                )
        ...
        if self.has_intermediate_head:
            ...
            iout = self.norm_intermediate_trunkfinal(
                iout, mask=orig_mask, mask_sum_hw=orig_mask_sum_hw, mask_sum=orig_mask_sum
            )
            ...
            with autocast("cuda", enabled=False):
                ...
                mask_sum_fp32 = (
                    orig_mask_sum.float()
                    if isinstance(orig_mask_sum, torch.Tensor)
                    else orig_mask_sum
                )
            ...
            for i, block in enumerate(
                self.blocks[self.intermediate_head_blocks :], start=self.intermediate_head_blocks
            ):
                if self.use_trunk_residual_backout:
                    out, backout = self._run_block_with_backout(
                        block,
        ...
        out = self.norm_trunkfinal(
            out, mask=orig_mask, mask_sum_hw=orig_mask_sum_hw, mask_sum=orig_mask_sum
        )
        ...
        with autocast("cuda", enabled=False):
            ...
            mask_sum_hw_fp32 = orig_mask_sum_hw.float()
            mask_sum_fp32 = (
                orig_mask_sum.float() if isinstance(orig_mask_sum, torch.Tensor) else orig_mask_sum
            )
            input_global_fp32 = input_global.float()
    ...
    def postprocess_single_heads_output(self, outputs):
        ...
        pred_scorestdev = (
            SoftPlusWithGradientFloorFunction.apply(out_miscvalue[:, 1], 0.05, False)
            * self.scorestdev_multiplier
        )
        pred_lead = out_miscvalue[:, 2] * self.lead_multiplier
        pred_variance_time = (
            SoftPlusWithGradientFloorFunction.apply(out_miscvalue[:, 3], 0.05, False)
            * self.variance_time_multiplier
        )
        if self.config["version"] < 14:
            pred_shortterm_value_error = (
                SoftPlusWithGradientFloorFunction.apply(out_moremiscvalue[:, 0], 0.05, False)
                * self.shortterm_value_error_multiplier
            )
            pred_shortterm_score_error = (
                SoftPlusWithGradientFloorFunction.apply(out_moremiscvalue[:, 1], 0.05, False)
                * self.shortterm_score_error_multiplier
            )
        else:
            pred_shortterm_value_error = (
                SoftPlusWithGradientFloorFunction.apply(out_moremiscvalue[:, 0], 0.05, True)
                * self.shortterm_value_error_multiplier
            )
            pred_shortterm_score_error = (
                SoftPlusWithGradientFloorFunction.apply(out_moremiscvalue[:, 1], 0.05, True)
                * self.shortterm_score_error_multiplier
            )
...
def build_katago():
    config = config_of_name["b6c96"]
    return Model(config, pos_len=7)


def example_input_katago():
    model = build_katago()
    n = 2
    input_spatial = torch.randn(n, *model.bin_input_shape)
    input_global = torch.randn(n, *model.global_input_shape)
    return (input_spatial, input_global)


MENAGERIE_ENTRIES = [
    ("KataGo (b6c96)", build_katago, example_input_katago, 2019, "REAL"),
]
