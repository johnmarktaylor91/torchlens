# SOURCE: vendored from JusperLee/Dual-Path-RNN-Pytorch @ master
# https://github.com/JusperLee/Dual-Path-RNN-Pytorch/blob/master/model/model_rnn.py
#
# Dual-Path RNN (Luo, Chen & Yoshioka, arXiv:1910.06379) speech separation
# TasNet variant. model/model_rnn.py is self-contained torch (its only
# external helper, `check_parameters` from utils/util.py, is a trivial
# parameter-count debug print not used by the forward pass and is dropped
# here). Vendored verbatim below; no architectural change.
import torch
...
def build_dprnn_tasnet():
    # Tiny config for tracing (repo default: in=256, out=64, hidden=128,
    # num_layers=6); shrunk here, K left small enough for a short waveform.
    return Dual_RNN_model(16, 8, 8, bidirectional=True, norm="ln", num_layers=1, K=8)


def example_input_dprnn_tasnet():
    return torch.rand(1, 64)


MENAGERIE_ENTRIES = [
    ("DPRNN-TasNet", "build_dprnn_tasnet", "example_input_dprnn_tasnet", 2019, "vendored-pytorch"),
]
