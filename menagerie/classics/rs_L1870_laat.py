# SOURCE: vendored from aehrc/LAAT @ master
# https://github.com/aehrc/LAAT
# Files vendored verbatim (architecture unmodified):
#   src/models/rnn.py -> RNN
#   src/models/attentions/attention_layer.py -> AttentionLayer
#   src/models/attentions/util.py -> init_attention_layer / perform_attention
#   src/models/embeddings/embedding_layer.py -> EmbeddingLayer
# Only change: the real Vocab (which loads word2vec/mimic files from disk) is
# replaced with a minimal in-memory shim exposing the exact interface the
# vendored code calls (n_words/n_labels/n_level/all_n_labels/word_embeddings),
# and args_parser's argparse Namespace is replaced with an equivalent
# SimpleNamespace built with the same default hyperparameters. No layer,
# module, or attention mechanism was altered.
import copy
...
# ---------------------------------------------------------------------------
# src/models/attentions/attention_layer.py (verbatim)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# src/models/attentions/util.py (verbatim)
# ---------------------------------------------------------------------------
def init_attention_layer(model):
    if model.args.joint_mode == "flat":
        ...
        model.linears = nn.ModuleList(
            [
                nn.Linear(model.output_size + model.vocab.n_labels(level))
...
# ---------------------------------------------------------------------------
# Minimal in-memory Vocab shim (construction-only; NOT part of the
# architecture). The real aehrc/LAAT Vocab loads word2vec + MIMIC-III files
# from disk; this shim exposes the identical interface the vendored RNN /
# AttentionLayer code calls against, with tiny random-initialised sizes.
# ---------------------------------------------------------------------------
class _TinyVocab:
    def __init__(self, vocab_size=64, embedding_size=16, n_labels=(20,)):
        self._vocab_size = vocab_size
        self._n_labels = list(n_labels)
        self.word_embeddings = None  # -> "rand" embedding_mode in EmbeddingLayer

    def n_words(self):
        return self._vocab_size

    def n_labels(self, level):
        return self._n_labels[level]

    def n_level(self):
        return len(self._n_labels)

    def all_n_labels(self):
        return self._n_labels


def _build_laat_args(attention_mode="label", joint_mode="hierarchical"):
...
def build_laat():
    """Real LAAT: bi-LSTM encoder + per-label soft ('label'/LAAT) attention,
    hierarchical joint mode over 2 label levels (as used for MIMIC-III ICD
    coding in the paper)."""
    vocab = _TinyVocab(vocab_size=64, embedding_size=16, n_labels=(10, 20))
    args = _build_laat_args(attention_mode="label", joint_mode="hierarchical")
    model = RNN(vocab, args)
    model.eval()
    return model


def example_input_laat():
    batch_size = 2
    seq_len = 12
    batch_data = torch.randint(0, 64, (batch_size, seq_len), dtype=torch.long)
    lengths = torch.tensor([seq_len, seq_len - 3], dtype=torch.long)
    return batch_data, lengths


MENAGERIE_ENTRIES = [
    ("LAAT", build_laat, example_input_laat, 2020, "vendored-pytorch"),
]
