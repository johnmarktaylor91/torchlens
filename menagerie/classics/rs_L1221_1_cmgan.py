# SOURCE: vendored from https://github.com/ruizhecao96/CMGAN @ main
# Files: src/models/generator.py, src/models/conformer.py (upstream conformer
# credited by CMGAN as https://github.com/lucidrains/conformer/blob/master/conformer/conformer.py)
# Only import/name changes made: relative imports flattened into this single
# module; architecture and forward logic are byte-for-byte the upstream code.
"""CMGAN (Conformer-based Metric GAN) generator, TSCNet.

INTERSPEECH 2022 speech-enhancement model: dense encoder + 4 Time-Frequency
Conformer Blocks (TSCB) operating on the compressed complex STFT, followed by
a mask decoder (magnitude mask) and a complex decoder (residual real/imag).
Only the generator (TSCNet) is staged here -- the discriminator is a metric
GAN critic, not part of the forward-inference architecture family.
"""
...
# ---------------------------------------------------------------------------
# conformer.py (upstream: lucidrains/conformer)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# generator.py (CMGAN TSCNet generator)
# ---------------------------------------------------------------------------
...
def build_cmgan():
    # Small num_channel/num_features for a fast trace; architecture unchanged.
    return TSCNet(num_channel=16, num_features=17)


def example_input_cmgan():
    # (batch, 2, T, F) compressed complex STFT: channel 0 = real, channel 1 = imag.
    # num_features=17 must match the frequency dim F here.
    return torch.randn(1, 2, 10, 17)


MENAGERIE_ENTRIES = [
    ("CMGAN", "build_cmgan", "example_input_cmgan", 2022, MENAGERIE_ZOO),
]
