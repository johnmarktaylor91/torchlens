# SOURCE: vendored from dlunion/DBFace @ master
# https://github.com/dlunion/DBFace/blob/master/model/DBFace.py
# Center-based anchor-free single-stage face+landmark detector, designed for NCNN
# mobile deployment. MobileNetV3-style (Block/SeModule/HSwish/HSigmoid) backbone
# feeding a small FPN (ContextModule/DetectModule) and three heads (heatmap, TLRB
# box regression, 5-point landmark). Code is copied verbatim from the real repo file
# (only the module-level docstring/header above and a tiny `build_dbface()`/
# `example_input_dbface()` pair for a small random-init instance were added; no
...
class CBNModule(nn.Module):
    def __init__(self, inchannel, outchannel=24, kernel_size=3, stride=1, padding=0, bias=False):
        ...
        self.conv = nn.Conv2d(
            inchannel, outchannel, kernel_size, stride, padding=padding, bias=bias
        )
