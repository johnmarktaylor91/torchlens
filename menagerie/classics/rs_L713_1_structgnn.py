...
#
# Struct2Seq / "StructGNN": Ingraham, Garg, Barzilay, Jaakkola, "Generative Models for
# Graph-Based Protein Design" (NeurIPS 2019). Graph-conditioned autoregressive language
# model for protein sequence design from backbone geometry: a k-NN structural graph is
# featurized (dihedral angles, pairwise distance RBFs, relative orientation quaternions,
# sinusoidal positional encodings) and consumed by a stack of graph-transformer encoder
# layers (unmasked self-attention over structural neighbors) followed by autoregressive
# graph-transformer decoder layers (causally-masked self-attention over the neighbor
# graph) that emit per-residue amino-acid log-probabilities. Predates and is
# architecturally distinct from the later ProteinMPNN (mpnn message-passing option is
# also ported below via `use_mpnn=True`, matching the upstream `MPNNLayer` alternative
# gated by the same flag in the real `Struct2Seq.__init__`).
#
# This is the REAL model code from `struct2seq/struct2seq.py`, `struct2seq/self_attention.py`,
# and `struct2seq/protein_features.py`, combined verbatim into one file (only the
# `matplotlib` import used exclusively by commented-out debug plotting blocks, and the
# `from .x import *` relative imports, were dropped/flattened -- no architectural code
# was altered). Depends only on torch/numpy, both base libs.
#
# ruff: noqa: E731, E741 -- matches original repo style (lambda-assign helper, `O` for
# the real paper's orientation-matrix notation); kept for fidelity to the real source.
...
# ---- struct2seq/self_attention.py (verbatim, flattened) ----------------------------
...
# ---- struct2seq/protein_features.py (verbatim, flattened) --------------------------
...
# ---- struct2seq/struct2seq.py (verbatim; forward() flattened, unused debugging /
# sequential-decoding / sampling methods dropped since they are not part of the
# single-pass forward architecture) -------------------------------------------------
...
def build_structgnn():
    # Real released config: node_features=edge_features=hidden_dim=128,
    # num_encoder_layers=num_decoder_layers=3, k_neighbors=30, vocab=20 (canonical AA
    # alphabet). Shrunk to hidden_dim=8, k_neighbors=6, 1 encoder/1 decoder layer for a
    # menagerie-scale trace; use_mpnn=False selects the real graph-transformer
    # (TransformerLayer) variant that is StructGNN's headline architecture (the
    # use_mpnn=True MPNNLayer path is the same upstream file's alternative ablation).
    return Struct2Seq(
        num_letters=20,
        node_features=8,
        edge_features=8,
...
def example_input_structgnn():
    torch.manual_seed(0)
    batch, n_res = 1, 12
    # X: backbone coordinates [B, L, 4 atoms (N, CA, C, O), 3]
    X = torch.randn(batch, n_res, 4, 3)
    # S: ground-truth amino-acid sequence indices [B, L]
    S = torch.randint(0, 20, (batch, n_res))
    # L: unused positional argument in the real forward() signature (kept for
    # signature fidelity with the upstream `Struct2Seq.forward(self, X, S, L, mask)`)
    L = torch.zeros(batch, n_res)
...
MENAGERIE_ENTRIES = [
    ("StructGNN", "build_structgnn", "example_input_structgnn", 2019, "vendored-pytorch"),
]
