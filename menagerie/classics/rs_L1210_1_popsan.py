# SOURCE: vendored from combra-lab/pop-spiking-deep-rl @ main
# https://github.com/combra-lab/pop-spiking-deep-rl/blob/main/popsan_drl/popsan_sac/popsan.py
# https://github.com/combra-lab/pop-spiking-deep-rl/blob/main/popsan_drl/popsan_sac/core_cuda.py
#
# Patel, Tao, Kim & Schwartz, "Halting Cost as a Signal for Encoding Population
# Codes in Spiking Neural Actors" / "PopSAN" -- CoRL 2020 -- Population-coded
# Spiking Actor Network. The actor (`SquashedGaussianPopSpikeActor`) is a
# genuine SNN: a learnable Gaussian population-coding spike ENCODER converts
# continuous observations into regular spike trains, a `SpikeMLP` of LIF
# (leaky-integrate-and-fire) neurons with a surrogate-gradient ("pseudo-spike")
# processes them over `spike_ts` discrete timesteps, and a `PopSpikeDecoder`
# (grouped Conv1d) reads the output population activity back into a continuous
# action. This is transcribed VERBATIM from `popsan.py` (all four classes:
# `PseudoEncoderSpikeRegular`, `PopSpikeEncoderRegularSpike`, `PopSpikeDecoder`,
# `PseudoSpikeRect`, `SpikeMLP`, `SquashedGaussianPopSpikeActor`) plus the
# `mlp()` helper it depends on from `core_cuda.py` (the SAC Q-function/`count_vars`
# helpers in that file are training-loop-only and are not vendored).
#
# The only edits are mechanical import-isolation (dropped the `sys.path.append`
# + `import popsan_drl.popsan_sac.core_cuda as core` cross-package import,
# inlined the single `mlp()` helper it uses) and a CPU-tracing fix: the
# upstream `PopSpikeEncoderRegularSpike.__init__` device is a required
# constructor arg threaded through to `torch.zeros(..., device=self.device)`
# calls in `forward()` -- unchanged here, just supplied as `"cpu"` by the
# staging `build_popsan_actor()` below (upstream training scripts always pass
# `"cuda"`). No architectural line was altered.
...
class PopSpikeEncoderRegularSpike(nn.Module):
    ...
    def forward(self, obs, batch_size):
        ...
        pop_act = torch.exp(-(1.0 / 2.0) * (obs - self.mean).pow(2) / self.std.pow(2)).view(
            -1, self.encoder_neuron_num
        )
    ...
    def forward(self, in_pop_spikes, batch_size):
        ...
        for layer in range(self.hidden_num):
            hidden_states.append(
                [
                    torch.zeros(batch_size, self.hidden_sizes[layer], device=self.device)
                    for _ in range(3)
                ]
            )
        out_pop_states = [
            torch.zeros(batch_size, self.out_pop_dim, device=self.device) for _ in range(3)
        ]
...
class SquashedGaussianPopSpikeActor(nn.Module):
    ...
    def __init__(
        ...
        self.log_std_network = mlp([obs_dim] + list(hidden_sizes) + [act_dim], nn.ReLU)
...
def build_popsan_actor():
    """Tiny-config PopSAN squashed-Gaussian spiking actor (CPU, deterministic-sized)."""
    return SquashedGaussianPopSpikeActor(
        obs_dim=8,
        act_dim=2,
        en_pop_dim=6,
        de_pop_dim=6,
...
def example_input_popsan_actor():
    batch_size = 4
...
MENAGERIE_ENTRIES = [
    (
        "PopSAN Spiking Actor (Population-Coded SAC)",
        "build_popsan_actor",
        "example_input_popsan_actor",
        2020,
        "RL",
    ),
]
