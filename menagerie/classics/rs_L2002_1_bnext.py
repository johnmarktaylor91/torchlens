# SOURCE: vendored from https://github.com/hpi-xnor/BNext @ main
#
# BNext (Guo, Guo, Cheng, Yu, Liu, Zhang, Xu, Sun, Lu, Yao. 2023, CVPR, "Join the
# High Accuracy Club on ImageNet with A Binary Neural Network Ticket"). Binary
# convolutional network (weights/activations constrained toward {-1, +1}) built from
# an "ELM Attention" block (binarized 3x3 conv attention with a learnable-bias sign
# activation) and an "Info-RCP" (information-recoupling) squeeze-and-expand gate that
# mixes the binarized branch back with the real-valued input before a residual add,
# stacked ResNet-style over configurable stage widths (tiny/small/middle/large).
#
# Vendored verbatim (architecture-relevant classes only) from the repo's own file:
#   https://raw.githubusercontent.com/hpi-xnor/BNext/main/src/bnext.py
#
# What is kept: every module class byte-for-byte (HardSigmoid, firstconv3x3,
# LearnableBias, HardSign, HardBinaryConv, SqueezeAndExpand, Attention, FFN_3x3,
# FFN_1x1, the four BasicBlock* variants, and BNext itself) and the real stage-width
# tables (stage_out_channel_tiny/small/middle/large).
#
# What is dropped (import plumbing, not architecture): `from torchinfo import summary`
# is unused in the vendored classes (only referenced by the repo's own un-called
# `if __name__ == "__main__"` smoke block, which is also dropped here) and torchinfo
# is not one of this environment's installed base libs -- dropping the import changes
# no computation.
#
...
class HardBinaryConv(nn.Module):
    ...
    def forward(self, x):
        ...
        y = F.conv2d(
            x, binary_weights, stride=self.stride, padding=self.padding, groups=self.groups
        )
...
class FFN_1x1(nn.Module):
    def __init__(
        self, inplanes, planes, stride=1, attention=True, drop_rate=0.1, infor_recoupling=True
    ):
...
class BasicBlock_No_Infor_Recoupling(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.1, mode="scale"):
        ...
        if mode == "scale":
            self.Attention = Attention(
                inplanes, inplanes, stride, None, drop_rate, infor_recoupling=False, groups=1
            )
        else:
            self.Attention = FFN_3x3(
                inplanes,
...
class BNext(nn.Module):
    def __init__(self, num_classes=1000, size="tiny", ELM_Attention=True, Infor_Recoupling=True):
        ...
        for i in range(len(stage_out_channel)):
            if i == 0:
                self.feature.append(
                    firstconv3x3(3, stage_out_channel[i], 1 if num_classes != 1000 else 2)
                )
            elif i == 1:
                self.feature.append(
                    (
...
def build_bnext():
    # tiny size, small num_classes for fast tracing; architecture unchanged.
    return BNext(num_classes=10, size="tiny")


def example_input_bnext():
    return torch.randn(1, 3, 32, 32)


MENAGERIE_ENTRIES = [
    ("BNext", "build_bnext", "example_input_bnext", 2023, "vendored-pytorch"),
]
