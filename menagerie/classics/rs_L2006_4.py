# SOURCE: vendored from https://github.com/meituan/YOLOv6 @ main
# (yolov6/layers/common.py + yolov6/models/efficientrep.py)
#
# EfficientRep (meituan/YOLOv6, Li et al. 2022/2023) is the hardware-aware
# rep-style ("structural reparameterization", RepVGG-derived) backbone used
# throughout the YOLOv6 family for real-time detection. The classes below
# are copied verbatim from the official repo's layer library
# (yolov6/layers/common.py, needed by EfficientRep for RepVGGBlock/RepBlock/
# SimSPPF/etc) and model file (yolov6/models/efficientrep.py), with only
# these minimal import fixes:
#   - "from yolov6.layers.common import ..." -> flattened into this module
#     (common.py's classes are now defined directly above EfficientRep)
#   - dropped "from yolov6.utils.general import download_ckpt": only used
#     inside common.py's DetectBackend.__init__ (an inference-time checkpoint
#     loader we never construct here); its own weight-download helper is
#     unrelated to model architecture.
#
# EfficientRep's real architectural contribution is the RepVGGBlock-based
# backbone with SimSPPF spatial-pyramid pooling; every layer/mechanism below
# is the actual YOLOv6 repo code, unmodified.
#
# `DetectBackend` (a checkpoint-loading inference wrapper unrelated to model
# architecture, needing os/Path/download_ckpt) is dropped entirely since we
# never construct it.
...
class ConvBNReLU(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size=3, stride=1, padding=None, groups=1, bias=False
    ):
        super().__init__()
...
class ScaleLayer(torch.nn.Module):
    ...
    def forward(self, inputs):
        if self.bias is None:
            ...
            return inputs * self.weight.view(1, self.num_features, 1, 1) + self.bias.view(
...
class DarknetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, expansion=0.5):
        ...
        self.conv_2 = DPBlock(
            in_channel=hidden_channels, out_channel=out_channels, kernel_size=kernel_size, stride=1
        )
...
# ---------------------------------------------------------------------------
# yolov6/models/efficientrep.py -- EfficientRep backbone (verbatim)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Real construction: YOLOv6-nano-scale channel/repeat config (channels_list
# and num_repeats mirror the repo's configs/yolov6n.py backbone args, at a
# reduced input resolution for a fast trace).
# ---------------------------------------------------------------------------
...
def example_efficientrep_input():
    return torch.randn(1, 3, 128, 128)


MENAGERIE_ENTRIES = [
    ("EfficientRep", "build_efficientrep", "example_efficientrep_input", "2022", "CV"),
]
