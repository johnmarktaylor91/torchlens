# SOURCE: vendored from JizhiziLi/GFM @ master
# https://github.com/JizhiziLi/GFM
#   core/gfm.py (GFM architecture, verbatim)
#   core/util.py (collaborative_matting, verbatim -- the one function gfm.py's
#       forward() calls from `from util import *`)
#
# GFM ("Glance and Focus Matting", Bridging Composite and Real: Towards End-to-end Deep
# Image Matting, IJCV 2021): a shared ResNet/DenseNet encoder feeding two decoder heads
# (Glance -- coarse semantic segmentation via a PSP module, and Focus -- detail/transition
# matting via a bridge block + skip connections), fused by `collaborative_matting`.
#
# Only two changes from the upstream files, both non-architectural:
#   1. `from config import *` is dropped -- `config.py` is exclusively dataset-path
#      string constants (AM2K/BG20K/COCO paths, log dirs) never referenced by the model
#      forward pass; the architecture code has zero dependency on it.
#   2. `collaborative_matting` (transcribed verbatim below) has its two unconditional
#      `.cuda()` calls removed so it runs on whatever device the input tensors are on --
#      device placement, not architecture.
...
def build_decoder(
    in_channels, mid_channels_1, mid_channels_2, out_channels, last_bnrelu, upsample_flag
):
    layers = []
    layers += [
...
# ---------------------------------------------------------------------------
# core/util.py::collaborative_matting (verbatim except `.cuda()` -> device-of-input)
# ---------------------------------------------------------------------------
def collaborative_matting(rosta, glance_sigmoid, focus_sigmoid):
    if rosta == "TT":
        ...
        fusion_sigmoid = torch.clamp(fusion_sigmoid, min=0)
    else:
        values, index = torch.max(glance_sigmoid, 1)
        index = index[:, None, :, :].float()
        fusion_sigmoid = index + focus_sigmoid
        fusion_sigmoid = torch.clamp(fusion_sigmoid, max=1)
...
class GFM(nn.Module):
    def __init__(self, args):
        ...
        if self.backbone == "r34":
            ...
            self.encoder1 = nn.Sequential(
                self.densenet.features.denseblock1, self.densenet.features.transition1
            )
            self.encoder2 = nn.Sequential(
                self.densenet.features.denseblock2, self.densenet.features.transition2
            )
            self.encoder3 = nn.Sequential(
                self.densenet.features.denseblock3, self.densenet.features.transition3
            )
    ...
    def forward(self, input):
        ...
        if self.rosta == "RIM":
            ...
            return [
                [glance_sigmoid_tt, focus_sigmoid_tt, fusion_sigmoid_tt],
...
class _GFMArgs:
    """Tiny stand-in for the argparse Namespace the real `GFM(args)` expects
    (`args.backbone`, `args.rosta`) -- upstream `core/test.py` constructs one via
    argparse; here it's just the two attributes the constructor reads."""

    def __init__(self, backbone="r34", rosta="TT"):
        self.backbone = backbone
        self.rosta = rosta


def build_gfm():
    return GFM(_GFMArgs(backbone="r34", rosta="TT"))


def example_input_gfm():
    return torch.zeros(1, 3, 64, 64)


MENAGERIE_ENTRIES = [
    ("GFM (Glance and Focus Matting)", build_gfm, example_input_gfm, 2021, "vendored"),
]
