# SOURCE: vendored from https://github.com/divelab/AIRS @ 4a16c68a7da707c521019067dec51c227c10de45 (main)
#
# QHNet: SE(3)-equivariant graph neural network that predicts the electronic
# Hamiltonian matrix of a molecule from its atomic geometry (OpenDFT/QHNet).
# Files combined (classes copied verbatim from the real repo, imports/paths fixed
# minimally so the module is self-contained):
#   - OpenDFT/QHNet/models/functional.py                                   -> cutoff_function, softplus_inverse
#   - OpenDFT/QHNet/models/modules/exponential_bernstein_radial_basis_functions.py
#                                                                           -> ExponentialBernsteinRadialBasisFunctions
#   - OpenDFT/QHNet/models/QHNet.py                                        -> NormGate, ConvLayer, InnerProduct,
#                                                                              ConvNetLayer, PairNetLayer,
#                                                                              SelfNetLayer, Expansion, QHNet
#
...
# ------------------------------------------------------------------
# OpenDFT/QHNet/models/functional.py  (verbatim)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# OpenDFT/QHNet/models/modules/exponential_bernstein_radial_basis_functions.py  (verbatim)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# OpenDFT/QHNet/models/QHNet.py  (verbatim)
# ------------------------------------------------------------------
def prod(x):
...
class NormGate(torch.nn.Module):
    def __init__(self, irrep):
        ...
        self.fc = nn.Sequential(nn.Linear(num_mul, num_mul), nn.SiLU(), nn.Linear(num_mul, num_mul))
...
class ConvLayer(torch.nn.Module):
    def __init__(
        ...
        self.irrep_in_node = (
            irrep_in_node if isinstance(irrep_in_node, o3.Irreps) else o3.Irreps(irrep_in_node)
        )
        self.irrep_hidden = (
            irrep_hidden if isinstance(irrep_hidden, o3.Irreps) else o3.Irreps(irrep_hidden)
        )
...
class InnerProduct(torch.nn.Module):
    def __init__(self, irrep_in):
        ...
        self.tp = o3.TensorProduct(
            self.irrep_in, self.irrep_in, irrep_out, instr, irrep_normalization="component"
        )
...
class PairNetLayer(torch.nn.Module):
    ...
    def forward(self, data, node_attr, node_pair_attr=None):
        ...
        node_pair = self.tp_node_pair(
            node_attr[src], node_attr[dst], self.fc_node_pair(data.full_edge_attr) * self.fc(s0)
        )
        ...
        for ins in self.instructions:
            ...
            if ins[3] is True or weights is not None:
                if weights is None:
                    ...
                    result = (
                        torch.einsum("wuv, ijk, bwk-> buivj", weight, w3j_matrix, x1)
                        / mul_ir_in.mul
                    )
                else:
                    weight = weights[
...
class QHNet(nn.Module):
    ...
    def forward(self, data, keep_blocks=False):
        ...
        _, full_edge_index, full_edge_attr, full_edge_sh, transpose_edge_index = self.build_graph(
            data, 10000
        )
    ...
    def build_graph(self, data, max_radius):
        ...
        for graph_idx in range(data.ptr.shape[0] - 1):
            ...
            transpose_index = (
                sub_graph_edge_index[0] * (num_nodes - 1) + sub_graph_edge_index[1] - bias
            )
                ...
                for dst_idx in range(data.ptr[graph_idx], data.ptr[graph_idx + 1]):
                    if src_idx == dst_idx:
                        matrix_col.append(
                            ...
                            .index_select(-2, self.orbital_mask[data.atoms[dst_idx].item()])
                            .index_select(-1, self.orbital_mask[data.atoms[src_idx].item()])
                        )
                    else:
                        mask1 = src == src_idx
                        mask2 = dst == dst_idx
...
# ------------------------------------------------------------------
class _TinyMolData:
    """Minimal stand-in for a torch_geometric Batch object carrying only the
    fields QHNet.forward/build_graph read: atoms, pos, batch, ptr.
    """

    def __init__(self, atoms, pos, batch, ptr):
        self.atoms = atoms
        self.pos = pos
        self.batch = batch
        self.ptr = ptr
...
def build_qhnet():
    ...
    model = QHNet(
        ...
        sh_lmax=2,
        hidden_size=8,
        bottle_hidden_size=8,
        num_gnn_layers=4,
        max_radius=8,
        num_nodes=11,
        radius_embed_dim=8,
    )
    model.set(torch.device("cpu"))  # mutates model.orbital_mask in place
    return model


def example_input_qhnet():
    torch.manual_seed(0)
    # tiny water molecule: O (atomic number 8), H, H -- single graph, one batch.
    ...
    return (_TinyMolData(atoms, pos, batch, ptr),)


MENAGERIE_ENTRIES = [
    ("qhnet", build_qhnet, example_input_qhnet, 2023, MENAGERIE_ZOO),
]
