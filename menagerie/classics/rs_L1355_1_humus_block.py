# SOURCE: vendored from https://github.com/AIF4S/HUMUS-Net @ master
#   Vendored file:
#     - models/humus_block.py -> full contents (Mlp, window_partition/window_reverse,
#       WindowAttention, SwinTransformerBlock, PatchMerging, PatchExpandSkip, BasicLayer,
#       RSTB, PatchEmbed(Learned)/PatchUnEmbed(Learned), HUMUSBlock, ConvBlock,
#       TransposeConvBlock, DownsampConvBlock) -- verbatim, only torch/timm/einops
#       dependencies (no changes needed; the file is already self-contained).
#
# HUMUS-Block is the "Hybrid Unrolled Multi-scale" denoiser regularizer at the core of
# HUMUS-Net (Fabian, Tinaz & Soltanolkotabi, NeurIPS 2022): a U-shaped hybrid
# convolutional + Swin-Transformer network. A strided-conv "MUST" front/back-end first
# downsamples into a multi-scale U-Net-like encoder-bottleneck-decoder built from
# Residual Swin Transformer Blocks (RSTB) with patch merging/expanding skip connections,
# then a symmetric conv-based upsampling tail reconstructs the residual. In the full
# HUMUS-Net (not vendored here -- it additionally requires the `fastmri` package for
# k-space data-consistency/VarNet cascading, complex arithmetic, and coil-sensitivity
# estimation, none of which are part of the novel architecture), HUMUSBlock is the
# per-cascade regularizer plugged into an unrolled gradient-descent MRI reconstruction
# loop. HUMUSBlock itself -- the actual novel contribution -- is reproduced verbatim
# and traced standalone here (2-channel real/imag image in, 2-channel image out).
...
class RSTB(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, embed_dim=dim, norm_layer=None
        )
...
class PatchEmbedLearned(nn.Module):
    ...
    def patch_to_vec(self, x):
        ...
        patches = (
            ...
            .contiguous()
            .view(-1, self.patch_size[0], self.patch_size[1], C)
        )
...
class DownsampConvBlock(nn.Module):
    ...
    def __init__(self, in_chans: int, out_chans: int):
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(in_chans, out_chans, kernel_size=2, stride=2, bias=False),
            nn.InstanceNorm2d(out_chans),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
...
def build_humus_block():
    """HUMUSBlock at tiny size for tracing. Architecture is unmodified from the real
    repo; only the size knobs (img_size/embed_dim/depths/num_heads/window_size/
    bottleneck) are shrunk to keep tracing cheap."""
    model = HUMUSBlock(
        img_size=(32, 32),
        in_chans=2,
        patch_size=1,
...
def example_input_humus_block():
    """Matches HUMUSBlock.forward: (B, in_chans=2, H, W) real/imag-as-channels image."""
    return torch.randn((1, 2, 32, 32), dtype=torch.float32)


MENAGERIE_ENTRIES = [
    ("HUMUS-Block", "build_humus_block", "example_input_humus_block", 2022, MENAGERIE_ZOO),
]
