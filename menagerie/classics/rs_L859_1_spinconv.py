# SOURCE: vendored from facebookresearch/fairchem @ v0.1.0 (ocpmodels/models/spinconv.py,
# the last release under the pre-fairchem_core "ocpmodels" package name; canonical origin
# Open-Catalyst-Project/ocp, later renamed/merged into facebookresearch/fairchem).
# Files combined from the real repo (paths as in upstream):
#   ocpmodels/models/spinconv.py    (spinconv, SpinConvBlock, MessageBlock, ForceOutputBlock,
#                                     EmbeddingBlock, DistanceBlock, ProjectLatLongSphere, Swish,
#                                     GaussianSmearing -- the paper's spherical message-passing
#                                     architecture, "SpinConv: Rotation-Invariant Spin-Convolution
#                                     for Molecular Property Prediction", Shuaibi et al. 2021/2022)
#   ocpmodels/models/base.py        (BaseModel.generate_graph -- graph construction glue)
#   ocpmodels/common/utils.py       (subset: conditional_grad, get_pbc_distances, radius_graph_pbc,
#                                     get_max_neighbors_mask, compute_neighbors -- the free-function
#                                     subset spinconv.py/base.py import; the rest of that ~1000-line
#                                     module is OCP-trainer/registry/matplotlib plumbing unrelated to
#                                     the model architecture and is not needed to run spinconv forward)
#
# Minimal import fixes only (no architecture changes): dropped `ocpmodels.common.registry`
# (@registry.register_model decorator, framework plumbing) and `ocpmodels.common.transforms
# .RandomRotate` (only used by OCP's training-time data augmentation pipeline, never inside the
# model's forward()). Constructed here with the real default `sphere_message="fullconv"` /
# `output_message="fullconv"` / `force_estimator="random"` config, which is the path that does NOT
# touch the optional e3nn-gated `rotspharmwd` spherical-harmonics branch (that branch is dead code
# unless `sphere_message="rotspharmwd"`, which we do not select) -- so no e3nn dependency is needed.
# Ran with `use_pbc=False, otf_graph=True` (spinconv's real non-periodic-graph configuration,
# radius_graph over raw positions) to avoid needing periodic-cell metadata for a single-molecule
# example input.
...
try:  # pragma: no cover - optional dep, only needed by the unused rotspharmwd path
    from e3nn import o3
...
# ---------------------------------------------------------------------------
# Vendored from ocpmodels.common.utils (facebookresearch/fairchem @ v0.1.0)
# Only the free functions spinconv.py / BaseModel actually need at forward
# time: conditional_grad, get_pbc_distances, radius_graph_pbc (+ its
# get_max_neighbors_mask helper), compute_neighbors. The rest of that module
# (config/logging/trainer plumbing) is unrelated to the model architecture.
# ---------------------------------------------------------------------------
...
def radius_graph_pbc(data, radius, max_num_neighbors_threshold, pbc=[True, True, False]):
...
# ---------------------------------------------------------------------------
# Vendored from ocpmodels.models.base.BaseModel (facebookresearch/fairchem @ v0.1.0)
# ---------------------------------------------------------------------------
...
class spinconv(BaseModel):
    ...
    def _filter_edges(self, edge_index, edge_distance, edge_distance_vec, max_num_neighbors):
        ...
        indices = indices + (torch.arange(len(indices), device=device) * max_neighbors).view(
            -1, 1
        ...
        edge_distance_vec = torch.masked_select(edge_distance_vec, edge_distance_vec_mask).view(
            -1, 3
        )
    ...
    def _project2D_edges_init(self, rot_mat, edge_index, edge_distance_vec):
        ...
        target_edge = (
            torch.arange(length, device=device).long().view(-1, 1).repeat(1, max_neighbors)
        )
...
def build_spinconv():
    """Tiny random-init spinconv model (non-periodic path, random-rotation force head)."""
    torch.manual_seed(0)
    return spinconv(
        num_atoms=None,
        bond_feat_dim=None,
        ...
        force_estimator="random",
        num_rand_rotations=2,
    )


def example_input_spinconv():
    """A small non-periodic molecular graph (torch_geometric Data-like object) -- 6 atoms."""
...
MENAGERIE_ENTRIES = [
    ("SpinConv", build_spinconv, example_input_spinconv, 2021, MENAGERIE_ZOO),
]
