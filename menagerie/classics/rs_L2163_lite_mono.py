# SOURCE: vendored from noahzn/Lite-Mono @ main
# https://raw.githubusercontent.com/noahzn/Lite-Mono/main/networks/depth_encoder.py
#
# Zhang, Chen, Wang, Wong 2023 (CVPR) "Lite-Mono: A Lightweight CNN and Transformer
# Architecture for Self-Supervised Monocular Depth Estimation" -- hybrid encoder combining
# Consecutive Dilated Convolutions (CDC, via DilatedConv) with Local-Global Features
# Interaction (LGFI, cross-covariance attention + Fourier positional encoding) blocks
# across 3 downsampling stages. LiteMono and its PositionalEncodingFourier/XCA/LayerNorm/
# BNGELU/Conv/CDilated/DilatedConv/LGFI/AvgPool submodules are copied verbatim from
# networks/depth_encoder.py (the encoder half of the full depth-estimation pipeline; the
# companion depth_decoder.py is a plain conv-upsample DispDecoder-style head not part of
# the novel architectural contribution). `timm.models.layers.DropPath` is imported via the
# non-deprecated `timm.layers.DropPath` path (identical implementation, avoids a
# FutureWarning in current timm).
"""Lite-Mono: CDC (dilated-conv) + LGFI (cross-covariance-attention) hybrid depth encoder."""
...
class CDilated(nn.Module):
    ...
    def __init__(self, nIn, nOut, kSize, stride=1, d=1, groups=1, bias=False):
        ...
        self.conv = nn.Conv2d(
            nIn, nOut, kSize, stride=stride, padding=padding, bias=bias, dilation=d, groups=groups
        )
...
class LGFI(nn.Module):
    ...
    def __init__(
        ...
        self.gamma_xca = (
            nn.Parameter(layer_scale_init_value * torch.ones(self.dim), requires_grad=True)
            if layer_scale_init_value > 0
            else None
        )
        self.xca = XCA(
            self.dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop
        )
...
def build_lite_mono():
    return LiteMono(in_chans=3, model="lite-mono-tiny", height=192, width=640)


def example_input_lite_mono():
    torch.manual_seed(0)
    return (torch.randn(1, 3, 192, 640),)


MENAGERIE_ENTRIES = [
    ("Lite-Mono", "build_lite_mono", "example_input_lite_mono", 2023, "vendored"),
]
