# SOURCE: vendored from AIRI-Institute/GENA_LM @ main
# File: src/gena_lm/modeling_bert.py
#
# GENA-LM (Fishman et al., NAR Genomic Bioinformatics 2023): a family of open-source DNA
# language models pretrained on the T2T human genome assembly (and multi-species genomes),
# supporting long input sequences (up to 36kb via BigBird sparse attention / Recurrent Memory
# Transformer wrappers). The base architecture in this file is a modified HuggingFace BERT
# (adapted from `transformers==4.17.0` modeling_bert.py) with two real, non-stock deviations
# used by the shipped checkpoints (e.g. AIRI-Institute/gena-lm-bert-base-lastln-t2t):
# (1) an optional **pre-layer-norm** transformer block (`config.pre_layer_norm=True`, Xiong et
# al. 2020 pre-LN placement -- LayerNorm moved before self-attention/FFN instead of after,
# implemented in `BertSelfOutput`/`BertOutput`/`BertLayer`/`BertEncoder`) with an optional
# final `last_layer_norm`; (2) optional block-sparse attention (`config.sparse_attention`,
# requires `deepspeed.ops.sparse_attention.SparseSelfAttention`, lazily imported only when that
# config key is set) and optional rotary position embeddings (`position_embedding_type=
# "rotary"`, `RotaryEmbedding` class below) as alternatives to absolute position embeddings.
# These deviations are genuine architectural modifications vs stock `transformers.BertModel`,
# so this cannot be built at rung 1 from the installed transformers BERT class.
#
# Verbatim except: sparse attention is left in place but unused (the default/absolute+pre-LN
# path below never sets `config.sparse_attention`, so the lazy `deepspeed` import is never
# exercised); no architecture lines were changed. Only base libs (`torch`, `transformers`,
# `packaging`) are required for the dense pre-LN path exercised here.
#
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None, has_relative_attention_bias=False):
        ...
        if self.is_sparse and self.position_embedding_type not in [
            "absolute",
            "relative_attention_bias",
            "rotary",
        ]:
            raise RuntimeError(
                f"SparseAttention supports `absolute`, `relative_attention_bias` and `rotary` position "
        ...
        if self.is_decoder and self.position_embedding_type == "relative_attention_bias":
            raise RuntimeError(
                f"BertSelfAttention does not support `relative_attention_bias` with `is_decoder` "
                f" = {self.is_decoder}"
            )
        ...
        if (
            ...
            self.position_embedding_type == "relative_attention_bias"
        ...
        if self.is_sparse:
            ...
            self.sparse_self_attention = SparseSelfAttention(
                self.sparse_config, max_seq_length=self.max_seq_len
            )
    ...
    def forward(
        ...
        if self.position_embedding_type == "rotary":
            ...
            if past_key_value is not None:
                raise RuntimeError(
                    f"past_key_values is not None are not supported in BertSelfAttention.forward with "
            ...
            if self.position_embedding_type in ["relative_key", "relative_key_query"]:
                ...
                if self.position_embedding_type == "relative_key":
                    ...
                    relative_position_scores_key = torch.einsum(
                        "bhdr,lrd->bhlr", key_layer, positional_embedding
                    )
                    attention_scores = (
                        attention_scores
                    ...
                    position_bias = self.get_relative_attention_bias(
                        position_bias, bs, seq_len, seq_len
                    )
        ...
        if self.is_sparse and output_attentions:
            ...
            raise RuntimeError(
                f"SparseAttention does not support output_attention = {output_attentions}"
            )
...
class BertLayer(nn.Module):
    ...
    def feed_forward_chunk(self, attention_output):
        intermediate_inp = (
            attention_output
            if not self.pre_layer_norm
            else self.post_attention_ln(attention_output)
        )
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [
                BertLayer(config, has_relative_attention_bias=bool(i == 0))
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.is_sparse and seq_length % self.sparse_block_size != 0:
            raise RuntimeError(
                f"BertModel with sparse attention is used, but seq_len = {seq_length} "
...
def build_gena_lm():
    # Real gena-lm-bert-base-lastln-t2t config fields kept as-is (pre_layer_norm=True,
    # last_layer_norm=True -- the checkpoint's distinguishing pre-LN + final-LN variant);
    # dims shrunk from the real hidden_size=768/num_hidden_layers=12/num_attention_heads=12/
    # intermediate_size=3072/vocab_size=32000/max_position_embeddings=512 to keep the trace
    # lightweight.
    config = BertConfig(
        vocab_size=64,
        hidden_size=32,
    ...
    config.pre_layer_norm = True
    config.last_layer_norm = True
    return BertForPreTraining(config)


def example_input_gena_lm():
    return torch.randint(0, 64, (1, 16))


MENAGERIE_ZOO = "vendored-pytorch"
MENAGERIE_ENTRIES = [
    ("GENA-LM", "build_gena_lm", "example_input_gena_lm", 2023, "vendored"),
]
