# SOURCE: vendored from chinitaberrio/BlockNeRFPytorch @ main (mega_nerf/models/nerf.py)
#
# chinitaberrio/BlockNeRFPytorch is a Block-NeRF PyTorch reimplementation whose actual
# per-block radiance-field network is the Mega-NeRF `NeRF` MLP defined in
# mega_nerf/models/nerf.py (positional-encoded xyz/view-direction MLP with an appearance
# embedding). This is the real per-block network used by the Block-NeRF pipeline; only
# imports/formatting are adjusted to be self-contained (removed cross-module Optional/List
# typing dependency on the rest of the package -- kept identical layer structure/forward
# logic). No architectural changes.
...
class NeRF(nn.Module):
    ...
    def forward(
        ...
        sigma_noise: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        expected = (
            self.xyz_dim
        ...
        return torch.cat(
            [self.rgb_activation(rgb) if self.rgb_activation is not None else rgb, sigma],
            -1,
        )
...
def build_block_nerf():
    """Tiny per-block Block-NeRF radiance MLP (xyz + view direction + appearance code)."""
    return NeRF(
        pos_xyz_dim=4,
        pos_dir_dim=2,
        layers=4,
        ...
        rgb_dim=3,
        xyz_dim=3,
        sigma_activation=ShiftedSoftplus(),
    )


def example_input_block_nerf():
    # x layout expected by NeRF.forward: [xyz(3) | dir(3) | appearance_idx(1)]
    return torch.rand(8, 3 + 3 + 1)


MENAGERIE_ENTRIES = [
    ("block_nerf_mlp", "build_block_nerf", "example_input_block_nerf", 2022, "vendored-pytorch"),
]
