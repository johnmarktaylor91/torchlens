# SOURCE: vendored from agrimgupta92/sgan @ master
# Files combined:
#   sgan/models.py (make_mlp, get_noise, Encoder, Decoder, PoolHiddenNet, SocialPooling,
#       TrajectoryGenerator, TrajectoryDiscriminator)
#
# Social GAN (Gupta et al., "Social GAN: Socially Acceptable Trajectories with Generative
# Adversarial Networks", CVPR 2018) predicts multi-agent pedestrian trajectories with an
# LSTM encoder-decoder generator conditioned on a max-pooled "social" context vector
# (PoolHiddenNet, the paper's pooling module) plus injected noise, trained adversarially
# against an LSTM-encoder discriminator. This file vendors TrajectoryGenerator (the
# generator half) exactly as in the repo's `sgan/models.py`.
#
# Import-only fixes applied (no architectural change):
#   - The upstream code hardcodes `.cuda()` in `Encoder.init_hidden`, `get_noise`, and
#     `TrajectoryGenerator.forward` (decoder_c init) because the repo assumes GPU training.
#     Each `.cuda()` call is replaced with a device-aware equivalent (using the module's own
#     parameter device / a passed-through device, no CPU/GPU behavior branching in the
#     architecture itself) so the model can trace on CPU. No layer, dimension, or control-flow
#     change beyond removing the hard CUDA dependency.
#
...
class Encoder(nn.Module):
    ...
    def forward(self, obs_traj):
        ...
        state_tuple = self.init_hidden(batch, device=obs_traj.device)
...
class SocialPooling(nn.Module):
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        for _, (start, end) in enumerate(seq_start_end):
            ...
            offset = torch.arange(0, total_grid_size * num_ped, total_grid_size).type_as(
                seq_start_end
            )
...
class TrajectoryGenerator(nn.Module):
    ...
    def add_noise(self, _input, seq_start_end, user_noise=None):
        ...
        if user_noise is not None:
            ...
            z_decoder = get_noise(noise_shape, self.noise_type, device=_input.device)
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, user_noise=None):
        ...
        decoder_c = torch.zeros(self.num_layers, batch, self.decoder_h_dim, device=obs_traj.device)
...
def example_input_sgan():
    # 2 sequences ("scenes") of 2 pedestrians each -> batch=4 total peds
    obs_len = 8
    obs_traj = torch.rand(obs_len, 4, 2)
    obs_traj_rel = torch.rand(obs_len, 4, 2)
    seq_start_end = torch.tensor([[0, 2], [2, 4]], dtype=torch.long)
    return (obs_traj, obs_traj_rel, seq_start_end)


MENAGERIE_ENTRIES = [
    ("Social-GAN", "build_sgan", "example_input_sgan", 2018, MENAGERIE_ZOO),
]
