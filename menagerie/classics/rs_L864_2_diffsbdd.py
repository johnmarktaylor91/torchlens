# SOURCE: vendored from https://github.com/arneschneuing/DiffSBDD @ 5d0d38d16c8932a0339fd2ce3f67ade98bbdff27
# (equivariant_diffusion/egnn_new.py + equivariant_diffusion/dynamics.py -- the real
# E(3)/SE(3)-equivariant "EGNNDynamics" noise-prediction network used inside DiffSBDD's
# pocket-conditioned diffusion model for structure-based drug design. The nn.Module
# architecture (GCL, EquivariantUpdate, EquivariantBlock, EGNN, GNN, EGNNDynamics) is
# untouched from the source. The single trivial helper actually used from
# `equivariant_diffusion/en_diffusion.py` (`EnVariationalDiffusion.remove_mean_batch`,
# a `scatter_mean`-based centering utility, NOT part of the architecture) is
# reimplemented with `torch_geometric.utils.scatter(..., reduce="mean")` instead of the
# original `torch_scatter.scatter_mean` dependency (mathematically identical op; only
# the scatter-mean *library provider* changes, not the model).
import math
...
def remove_mean_batch(x, indices):
    mean = _tg_scatter(x, indices, dim=0, reduce="mean")
    x = x - mean[indices]
    return x


# ---------------------------------------------------------------------------
# equivariant_diffusion/egnn_new.py (verbatim)
# ---------------------------------------------------------------------------
class GCL(nn.Module):
    def __init__(
        self,
...
# ---------------------------------------------------------------------------
# equivariant_diffusion/dynamics.py (verbatim)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
def build_diffsbdd():
    # Tiny EGNNDynamics network, matching the real DiffSBDD
    # `EGNNDynamics(mode='egnn_dynamics')` constructor signature.
    return EGNNDynamics(
        atom_nf=6,
        residue_nf=10,
        n_dims=3,
...
def example_input_diffsbdd():
    torch.manual_seed(0)
    n_atoms, n_residues = 5, 4
    atom_nf, residue_nf, n_dims = 6, 10, 3
    xh_atoms = torch.randn(n_atoms, n_dims + atom_nf)
    xh_residues = torch.randn(n_residues, n_dims + residue_nf)
    t = torch.full((n_atoms + n_residues, 1), 0.5)
...
MENAGERIE_ENTRIES = [
    ("DiffSBDD", build_diffsbdd, example_input_diffsbdd, 2023, "SOURCE_AVAILABLE"),
]
