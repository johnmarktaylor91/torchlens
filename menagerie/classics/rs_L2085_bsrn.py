# SOURCE: vendored from xiaom233/BSRN @ main
# https://raw.githubusercontent.com/xiaom233/BSRN/main/basicsr/archs/BSRN_arch.py
# https://raw.githubusercontent.com/xiaom233/BSRN/main/basicsr/archs/Upsamplers.py
#
# Li, Zeyuan / Liu, Yingqi / Chen, Xiangyu / Cai, Haoming / Gu, Jinjin / Qiao, Yu /
# Dong, Chao (2022) "Blueprint Separable Residual Network for Efficient Image
# Super-Resolution" (CVPR 2022 Workshops, NTIRE efficient-SR-track winner, official
# repo xiaom233/BSRN). BSRN's contribution is architectural: Blueprint Separable
# Convolutions (BSConvU, pointwise-then-depthwise factorized conv) inside an
# Efficient Separable Distillation Block (ESDB, an information-distillation block
# with progressive channel splitting), each ESDB followed by an Enhanced Spatial
# Attention (ESA) module and a Contrast-aware Channel Attention (CCALayer) module,
# stacked 8x with a final PixelShuffleDirect upsampler -- this is a genuine new
# building block (BSConv+ESA+ESDB), not a stock library class, so it is vendored
# verbatim (real code) rather than built from a library class.
#
# `basicsr/archs/BSRN_arch.py` and the `PixelShuffleDirect`/`UpsampleOneStep`
# classes from `basicsr/archs/Upsamplers.py` are reproduced below verbatim. Only
# the `@ARCH_REGISTRY.register()` decorator (basicsr's internal registry, not
# needed outside the basicsr training framework) and the `import basicsr.archs...`
# / `from basicsr.utils.registry import ARCH_REGISTRY` lines are dropped since we
# do not vendor the basicsr package; every remaining line of module code is
# unmodified from the official repo.
...
# ============================================================================
# basicsr/archs/Upsamplers.py :: UpsampleOneStep, PixelShuffleDirect (verbatim)
# ============================================================================
...
# ============================================================================
# basicsr/archs/BSRN_arch.py (verbatim)
# ============================================================================
...
class BSConvS(torch.nn.Module):
    ...
    def _reg_loss(self):
        ...
        I = torch.eye(WWt.shape[0], device=WWt.device)  # noqa: E741 (verbatim from real repo)
...
def build_bsrn():
    # Tiny random-init config: narrower feature channels than the published
    # 64-channel/8-block model, same architecture (BSConvU + ESDB + ESA +
    # CCALayer stack + PixelShuffleDirect x4 upsampler). num_feat=16 is the
    # floor for CCALayer's default reduction=16 (channel // reduction must
    # stay >= 1); num_block stays at the published 8 (ESDB stage count is an
    # architectural choice, not a size knob to shrink).
    model = BSRN(
        num_in_ch=3,
        num_feat=16,
        num_block=8,
...
def example_input_bsrn():
    return torch.randn(1, 3, 16, 16)


MENAGERIE_ENTRIES = [
    ("BSRN", "build_bsrn", "example_input_bsrn", 2022, MENAGERIE_ZOO),
]
