# SOURCE: vendored from xinghaochen/TinySAM @ main
#
# TinySAM: Pushing the Envelope for Efficient Segment Anything Model
# Han Shu, Wenshuo Li, Yehui Tang, Yiman Zhang, Yihao Chen, Houqiang Li,
# Yunhe Wang, Xinghao Chen. AAAI 2025.
# https://github.com/xinghaochen/TinySAM
#
# TinySAM keeps the real SAM prompt-encoder/mask-decoder/two-way-transformer
# (facebookresearch/segment-anything lineage) but swaps the ViT-H image encoder for the
# real distilled TinyViT-5M encoder (embed_dims=[64,128,160,320], depths=[2,2,6,2]) --
# exactly the `build_sam_vit_t` construction in the real repo's `tinysam/build_sam.py`.
# The classes below (`TinyViT`, `PromptEncoder`, `MaskDecoder`, `TwoWayTransformer`,
# `Sam`, `LayerNorm2d`, `MLPBlock`) are vendored verbatim from
# `tinysam/modeling/{tiny_vit_sam,prompt_encoder,mask_decoder,transformer,common,sam}.py`
# (imports/module layout adjusted only; every layer/mechanism -- MBConv patch embed,
# windowed local attention + depthwise local_conv, two-way cross-attention mask decoder --
# is unmodified). `Sam.forward` in the real repo takes `List[Dict]` batched prompts
# (image + point/box/mask keys), so this staging module keeps that exact real forward
# path and wraps it in a thin `build_tinysam()` / `example_input_tinysam()` pair that
# supplies a tiny synthetic image + one point prompt, matching `tinysam.build_sam.
# build_sam_vit_t()`'s real construction (shrunk image size only, for fast tracing).
...
# ---------------------------------------------------------------------------
# vendored from tinysam/modeling/common.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# vendored from tinysam/modeling/tiny_vit_sam.py (real TinyViT-5M distilled encoder)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# vendored from tinysam/modeling/prompt_encoder.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# vendored from tinysam/modeling/transformer.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# vendored from tinysam/modeling/mask_decoder.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# vendored from tinysam/modeling/sam.py
# ---------------------------------------------------------------------------
class Sam(nn.Module):
...
# ---------------------------------------------------------------------------
# staging: real TinySAM `build_sam_vit_t()` construction, shrunk image size only
# ---------------------------------------------------------------------------
def build_tinysam():
    ...
    model = Sam(
        ...
        mask_decoder=MaskDecoder(
            ...
            transformer=TwoWayTransformer(
                depth=2, embedding_dim=prompt_embed_dim, mlp_dim=256, num_heads=8
            ),
...
def example_input_tinysam():
    image = torch.rand(3, 64, 64) * 255.0
    point_coords = torch.tensor([[[32.0, 32.0]]])
...
MENAGERIE_ENTRIES = [
    (
        "TinySAM (TinyViT-5M SAM)",
        "build_tinysam",
        "example_input_tinysam",
        2025,
        MENAGERIE_ZOO,
    ),
]
