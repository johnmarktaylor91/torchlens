# SOURCE: vendored from opendilab/InterFuser @ main
# (interfuser/timm/models/interfuser.py)
"""InterFuser (CoRL 2022) -- safety-enhanced autonomous driving with transformer.

InterFuser fuses multi-camera RGB (front/left/right/center) + LiDAR BEV into a shared
transformer encoder-decoder that jointly predicts waypoints, traffic-object density maps,
junction/traffic-light/stop-sign state. The real model class `Interfuser` lives in the
repo's own vendored fork of timm (`interfuser/timm/models/interfuser.py`); it imports
`.registry.register_model`, `.resnet.{resnet18d,resnet26d,resnet50d,resnet101d,...}` and
`.layers.{StdConv2dSame,StdConv2d,to_2tuple}` from that vendored timm fork. All of those
names exist with matching public behavior in current pip `timm` (verified: `resnet18d`/
`resnet26d`/`resnet50d` construct with `features_only=True, out_indices=[4]` identically,
`timm.layers.{StdConv2dSame,StdConv2d,to_2tuple}` are the same helpers), so this vendors
the `Interfuser` model class and its supporting transformer/waypoint-head modules VERBATIM
(architecture untouched) and only substitutes the vendored-fork imports for the real,
currently-installed `timm` package. `@register_model` is dropped (registry side-effect
only, not part of the forward computation) and its import is not used.

`interfuser_baseline()` is the repo's own default constructor
(`enc_depth=6, dec_depth=6, embed_dim=256, rgb_backbone_name="r50", lidar_backbone_name="r18",
waypoints_pred_head="gru", use_different_backbone=True`); `img_size` is shrunk from the
repo default (224) to 32 purely to keep the tiny trace fast -- `HybridEmbed` recomputes
patch-grid size dynamically from a dummy backbone forward at whatever `img_size` is passed,
so this changes resolution, not architecture.
"""
...
from timm.layers import StdConv2d, StdConv2dSame, to_2tuple  # noqa: F401
from timm.models import resnet18d, resnet26, resnet26d, resnet50, resnet50d, resnet101d
...
# ---------------------------------------------------------------------------
# interfuser/timm/models/interfuser.py -- vendored verbatim below (imports only fixed)
# ---------------------------------------------------------------------------
...
class SpatialSoftmax(nn.Module):
    ...
    def forward(self, feature):
        ...
        feature_keypoints = torch.stack(
            [
                feature_keypoints[:, :, 0] * 12,
                (feature_keypoints[:, :, 1] - 1) * 12,
            ],
            dim=-1,
        )
...
class GRUWaypointsPredictorWithCommand(nn.Module):
    def __init__(self, input_dim, waypoints=10):
        ...
        self.grus = nn.ModuleList(
            [torch.nn.GRU(input_size=input_dim, hidden_size=64, batch_first=True) for _ in range(6)]
        )
        self.encoder = nn.Linear(2, 64)
...
def build_attn_mask(mask_type):
    mask = torch.ones((151, 151), dtype=torch.bool)
...
class Interfuser(nn.Module):
    def __init__(
        ...
        if self.end2end:
            ...
            self.waypoints_generator = MultiPath_Generator(embed_dim + 32, embed_dim, 10)
    ...
    def forward(self, x):
        ...
        if self.direct_concat:
            ...
            left_image = torch.nn.functional.interpolate(left_image, size=(img_size, img_size))
            right_image = torch.nn.functional.interpolate(right_image, size=(img_size, img_size))
        ...
        if self.end2end:
            ...
            tgt = self.position_encoding(torch.ones((bs, 1, 20, 20), device=x["rgb"].device))
            tgt = tgt.flatten(2)
...
# (64 keeps the r50/r18 stride-32 feature maps >= 2x2 so BatchNorm2d is well-defined at
# batch size 1 in eval mode)


def build_interfuser():
    model = interfuser_baseline(img_size=_IMG_SIZE)
    model.eval()
    return model


def example_input_interfuser():
    bs = 1
    rgb = torch.randn(bs, 3, _IMG_SIZE, _IMG_SIZE)
    ...
    lidar = torch.randn(bs, 3, _IMG_SIZE, _IMG_SIZE)
    # measurements: [:6] one-hot command mask, [6] velocity scalar
    measurements = torch.zeros(bs, 7)
    measurements[:, 0] = 1.0
    measurements[:, 6] = 0.3
...
MENAGERIE_ENTRIES = [
    ("InterFuser", build_interfuser, example_input_interfuser, 2022, "vendored-pytorch"),
]
