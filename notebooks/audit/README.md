...
| `14_huggingface_guarded` | Guarded HF text + vision trace (tiny models), `tl.autoroute.input` detector registry, `tl.bridge.hf.trace_text`, real attention q/k/v head facets via `tl.facets`/`tl.head` | yes | `tl.bridge.hf`/`tl.autoroute` not in `tl.__all__`; `Facet.value` (not `.tensor`); SDPA facets need eager mode |
