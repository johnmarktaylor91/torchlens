...
- **Submodule families:** `tl.debug` 8/8 · `tl.export` 17/17 (12 dep-free formats
  ...
  profiler in depth) · collapse-v2 (`collapse=` float slider, `fold_repeats=`,
...
| `11_visualization_in_workflow` | `draw` option sweep, **collapse-v2** (string + float slider, `fold_repeats=`, `collapse_plan()`, `collapse_schedule()`), customization callbacks (`node_spec_fn`/`skip_fn`/`collapse_fn`), presentation knobs (`for_paper`, `node_label_fields`, `vis_intervention_mode`), `Trace.annotate`, `tl.viz` primitives (all 11 names) | yes | option sweeps tripwired for byte-identical renders (`fold_repeats`, `show_containers`, `node_mode='attention'` fallback); `viz.__all__` lists `batch_summary` (a module, not a callable); `node_overlay` rejects callables; `vis_outpath` double-extension footgun |
...
| `17_receptive_projective_fields` | **`tl.receptive_field`** (`cross_validate`, `verify`, `node_spec`), entity `receptive_field` / `projective_field` views, geometry/gradient/viz actions, layer-to-layer selectors, plural trace tables, and `tl.validate(scope="receptive_field")` | yes | None observed; this notebook asserts the ResNet-18 435 x 435 differentiator and preserves any future tripwire failure as a GAP |
