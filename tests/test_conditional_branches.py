...
def test_selective_save_preserves_conditional_metadata_for_unsaved_layers() -> None:
    ...
    assert all(
        bool_layer in log.layer_labels
        for event in log.conditional_records
        for bool_layer in event.bool_layers
    )
