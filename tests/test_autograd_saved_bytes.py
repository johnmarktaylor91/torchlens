...
def _non_source_passes(trace: Trace) -> list[OpLog]:
...
def test_autograd_saved_bytes_basic_shape_model() -> None:
    ...
    linear_passes = [layer for layer in trace.layer_list if layer.layer_type == "linear"]
    relu_passes = [layer for layer in trace.layer_list if layer.layer_type == "relu"]
...
def test_add_op_reports_zero_autograd_saved_bytes() -> None:
    ...
    assert trace.layer_logs[add_pass.layer_label_no_pass].autograd_saved_bytes == 0
    assert trace.total_autograd_saved_bytes == 0
...
def test_no_grad_sets_autograd_saved_fields_to_none() -> None:
    ...
    assert all(layer.autograd_saved_bytes is None for layer in trace.layer_list)
    assert all(layer.autograd_saved_tensor_count is None for layer in trace.layer_list)
    assert all(layer.autograd_saved_bytes is None for layer in trace.layer_logs.values())
    assert all(layer.autograd_saved_tensor_count is None for layer in trace.layer_logs.values())
    assert trace.total_autograd_saved_bytes is None
...
def test_requires_grad_false_sets_autograd_saved_fields_to_none() -> None:
    ...
    assert all(layer.autograd_saved_bytes is None for layer in _non_source_passes(trace))
    assert all(layer.autograd_saved_tensor_count is None for layer in _non_source_passes(trace))
    assert all(layer.autograd_saved_bytes is None for layer in trace.layer_logs.values())
    assert trace.total_autograd_saved_bytes is None
...
def test_layer_log_autograd_saved_rollup_matches_pass_values() -> None:
    ...
    for pass_log in _non_source_passes(trace):
...
def test_trace_autograd_saved_rollup_matches_layer_values() -> None:
    ...
    assert trace.total_autograd_saved_bytes == _sum_layer_autograd_bytes(trace)
...
def test_autograd_saved_fields_roundtrip_through_bundle_save_load(tmp_path: Path) -> None:
    ...
    assert loaded.total_autograd_saved_bytes == trace.total_autograd_saved_bytes
