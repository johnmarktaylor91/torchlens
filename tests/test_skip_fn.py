...
def _first_op_label(log: tl.Trace, layer_type: str) -> str:
    """Return the first op label with a matching layer type."""

    for layer_log in log.layer_logs.values():
        if layer_log.layer_type == layer_type:
            return layer_log.ops[0].label
    raise AssertionError(f"No layer with type {layer_type}")


def _render_id(label: str) -> str:
    """Return the DOT node id for a TorchLens label."""

    return label.replace(":", "pass")


def _op_labels(log: tl.Trace, layer_type: str) -> list[str]:
    """Return first-pass op labels for layers with a matching layer type."""

    return [
        layer_log.ops[0].label
        for layer_log in log.layer_logs.values()
        if layer_log.layer_type == layer_type
    ]
...
def test_skip_single_layer_chains_edge(tmp_path: Any) -> None:
    ...
    conv_label = _first_op_label(log, "conv2d")
    linear_label = _first_op_label(log, "linear")
    ...
    assert f"{_render_id(conv_label)} -> {_render_id(linear_label)}" in dot
...
def test_skip_branching(tmp_path: Any) -> None:
    ...
    source_label = _first_op_label(log, "linear")
    branch_labels = _op_labels(log, "linear")[1:]
    ...
    assert "relu_1_2 [" not in dot
    assert f"{_render_id(source_label)} -> {_render_id(branch_labels[0])}" in dot
    assert f"{_render_id(source_label)} -> {_render_id(branch_labels[1])}" in dot
