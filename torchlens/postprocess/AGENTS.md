...
- Step 6 mutates `equivalence_class`; it must precede Step 8.
...
- Step 16.5 must precede Step 17 because `ModuleLog.layers` points to `LayerLog` keys.
- Step 17.5 computes `graph_shape_hash` before `_set_tracing_finished` changes access behavior.
- Steps 19-20 are only for streamed out bundles.
...
`equivalence_class` so identical ops in different modules do not get loop-grouped
