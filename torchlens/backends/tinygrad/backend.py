...
def _enrich_tinygrad_op(op_log: Any) -> None:
    ...
    ----------
    op_log:
        Operation log receiving tinygrad dtype, device, and address metadata.

    Returns
    -------
    None
        ``op_log`` is mutated in place.
    """
...
def _enrich_tinygrad_layer(layer_log: Any, op_log: Any) -> None:
    """Copy tinygrad backend identity refs from an op to its layer log.

    Parameters
    ----------
    layer_log:
        Layer log created from ``op_log``.
    op_log:
        Finalized operation log with tinygrad identity refs.

    Returns
    -------
    None
        ``layer_log`` is mutated in place.
    """
...
def _tinygrad_metadata_top_level(
    address: str,
    metadata: dict[str, Any],
    metadata_by_address: dict[str, dict[str, Any]],
) -> bool:
    """Return whether a tinygrad metadata address is top-level.
...
def _tinygrad_op_top_level(address: str) -> bool:
    """Return whether a tinygrad op module address is top-level.
    ...
    -------
    bool
        True for every non-root first frame, matching the previous tinygrad loop.
    """

    return address != "self"


def _tinygrad_training_mode(metadata: dict[str, Any]) -> bool:
    """Return tinygrad module training state from metadata.

    Parameters
    ----------
    metadata:
        Module metadata from ``TinygradModuleTree``, unused by tinygrad.

    Returns
    -------
    bool
        Always ``False`` because tinygrad does not expose module training mode.
    """
