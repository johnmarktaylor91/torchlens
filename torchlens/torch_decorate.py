...
def torch_func_decorator(func: Callable, model_history: ModelHistory):
    @wraps(func)
    def wrapped_func(*args, **kwargs):
        ...
        output_tensors = get_vars_of_type_from_obj(
            out_orig, which_type=torch.Tensor, subclass_exceptions=[torch.nn.Parameter]
        )

        if len(output_tensors) > 0:
            model_history.log_function_output_tensors(
                func,
                args,
                kwargs,
                arg_copies,
...
def decorate_tensors(
    ...
    """
    namespace_name_notensor = namespace_name.replace("torch.Tensor.", "").replace(
        "torch.Tensor", ""
    )
