...
class ModelHistory:
    ...
    def get_op_nums_from_user_labels(
        self, which_layers: List[Union[str, int]]
    ) -> List[int]:
    ...
    def log_source_tensor(
        self, t: torch.Tensor, source: str, buffer_addr: Optional[str] = None
    ):
        ...
        for a, arg in enumerate(list(args) + list(kwargs.values())):
            if hasattr(arg, "tl_tensor_label_raw"):
                ...
                for arg_elem in arg_iter:
                    if not hasattr(arg, "tl_tensor_label_raw") and not isinstance(
    ...
    def postprocess(
        self, decorated_func_mapper: Dict, mark_input_output_distances: bool = True
    ):
        """
    ...
    def _add_output_layers(self):
        ...
        for i, output_layer_label in enumerate(self.output_layers):
            ...
            new_output_node.tensor_label_raw = (
                f"output_{i + 1}_{self.tensor_counter}_raw"
            )
        ...
        for node_label in current_iso_nodes:
            ...
            for node_type in node_types_to_use:
                ...
                for neighbor_label in getattr(node, node_type_field):
                    ...
                    ):  # if hit another subgraph, mark them adjacent & skip
    ...
    def _remove_unwanted_entries_and_log_remaining(self):
        ...
        for tensor_entry in self:
            ...
            if (
                tensor_entry.has_saved_activations
                or self.keep_layers_without_saved_activations
            ):
                # Add the lookup keys for the layer, to itself and to ModelHistory:
                self._add_lookup_keys_for_tensor_entry(
                    tensor_entry, i, self.num_tensors_saved
                )
        ...
        if (
            self.num_tensors_saved == len(self)
        ) or self.keep_layers_without_saved_activations:
    ...
    def _undecorate_all_saved_tensors(self, decorated_func_mapper: Dict):
        ...
        for layer_label in self.layer_labels:
            ...
            tensors_to_undecorate.extend(
                get_vars_of_type_from_obj(
                    tensor_entry.creation_args, torch.Tensor, search_depth=4
                )
            )
            tensors_to_undecorate.extend(
                get_vars_of_type_from_obj(
                    tensor_entry.creation_kwargs, torch.Tensor, search_depth=4
                )
            )
    ...
    def render_graph(
        ...
        if not self.all_layers_logged:
            raise ValueError(
                "Must have all layers logged in order to render the graph; either save "
    ...
    def _construct_layer_node(self, node, graphviz_graph, vis_opt):
    ...
    def _add_edges_for_node(
        ...
        for child_layer_label in parent_node.child_layers:
            ...
            if (child_layer_label in parent_node.cond_branch_start_children) and (
                not child_is_collapsed_module
            ):
    ...
    def _label_rolled_pass_nums(
        ...
        if (
            child_node.edges_vary_across_passes
            and (not child_node.is_output_layer)
    ...
    def validate_saved_activations(
        ...
        for i, output_layer_label in enumerate(self.output_layers):
            ...
            if not torch.equal(
                output_layer.tensor_contents, ground_truth_output_tensors[i]
            ):
                print(
    ...
    def _check_whether_func_on_saved_parents_yields_saved_tensor(
        ...
        if (
            not (
                torch.equal(
                    recomputed_output, layer_to_validate_parents_for.tensor_contents
                )
            )
            and not perturb
        ):
            ...
            for (
                ...
                if parent_layer_arg in layers_to_perturb:
                    parent_layer_func_values = self._perturb_layer_activations(
                        self[parent_layer_arg].tensor_contents
                    )
                else:
                    parent_layer_func_values = safe_copy(
                        self[parent_layer_arg].tensor_contents
                    )
    ...
    def _perturb_layer_activations(activations: torch.Tensor) -> torch.Tensor:
        ...
        if activations.dtype in [
            ...
            if len(tensor_unique_vals) > 1:
                ...
                perturbed_activations = torch.randint(
                    -10, 11, size=activations.shape, device=device
                )
        elif activations.dtype == torch.bool:
            perturbed_activations = torch.randint(
                0, 2, size=activations.shape, device=device
            ).bool()
        else:
