...
| 1 | fixup | pre-merge regression fixups | this commit | PASS: fixed four merge-blocking regressions in backend literal routing, MLX audit-only metadata save/load, torch SDPA facet parameter resolution after param-ref release, and fastlog code-context qualname caching. Gates: focused four-regression command, ruff, touched mypy, backend_parity, nearby modules, smoke. | Ready for pre-merge review; no threshold relaxation used. |
| 1 | fixup | F-IO audit-only portable-save regression | this commit | PASS: restored clean JAX/tinygrad audit-only portable-save rejection before torch blobification, kept explicit audit saves metadata-only, preserved MLX audit-null save/load behavior, and added regression assertions against the generic blobification error. Gates: ruff, touched mypy, smoke, backend_parity, full JAX venv, full tinygrad venv, MLX audit-only. | Ready for review; non-torch payload codecs remain future work. |
