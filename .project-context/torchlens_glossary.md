...
- `tl.trace(model, x, *, backend=None)`: Resolve a backend, run a captured forward pass, and return a `Trace`. `None` keeps torch eager capture as the default and MLX module auto-routing as a technical preview; explicit `backend="jax"`, `backend="tinygrad"`, and `backend="paddle"` enable their preview captures.
- `BackendName`: Public backend identifier used by `backend=` and `Trace.backend`. Shipped names are `"torch"`, `"mlx"`, `"jax"`, `"tinygrad"`, and `"paddle"`; `"fake"` is test-only.
...
- `trace.derived_grads`: Backend-neutral accessor for leaf-level gradients derived outside true backward capture. JAX populates it through a second `jax.value_and_grad` run; tinygrad populates it through a bracketed `DEV=PYTHON` leaf-gradient run; MLX populates it through a guarded `mx.value_and_grad` run; Paddle populates it through a guarded Paddle AD run.
- `trace.intermediate_derived_grads`: Accessor for exact per-op gradients derived outside true backward capture. JAX populates it through an opt-in separate zero-tap AD replay plus per-boundary oracle; tinygrad populates it through the separate no-realize intermediate-gradient pass; MLX populates it through an opt-in custom-VJP-tap AD replay plus per-record oracles; Paddle populates it through an opt-in guarded AD replay plus per-record oracles. It does not populate `trace.saved_grad_ops`.
- `op.derived_grad`: Read-only convenience property for an Op's exact `status == "exact"` entry in `trace.intermediate_derived_grads`; `op.grads` remains true-backward-only and raises on non-torch traces.
- `tl.backends.jax.GradOptions`: JAX preview options object passed to `tl.trace(..., backend="jax", grad_options=...)`; declares params, optional loss function, input-relative gradient argnums, and optional capped intermediate derived gradients.
- `tl.backends.mlx.GradOptions`: MLX preview options object passed to `tl.trace(..., backend="mlx", grad_options=...)`; declares optional loss function, input-relative gradient argnums, and optional capped custom-VJP-tap intermediate derived gradients.
- `tl.backends.paddle.GradOptions`: Paddle preview options object passed to `tl.trace(..., backend="paddle", grad_options=...)`; declares optional loss function, input-relative gradient argnums, and optional capped intermediate derived gradients.
...
- `backend`: Public `BackendName` tag identifying the capture backend, e.g. `"torch"`, `"mlx"`, `"jax"`, `"tinygrad"`, or `"paddle"`. Survives portable save/load.
...
- `jax_control_flow`: Public `trace()` option for JAX control-flow handling; `lax.scan`/`cond`/`while` can be unrolled for raw JAX function-root captures, and `"region"` forces supported scan/while/custom-VJP-forward boundaries to metadata-first regions.
- `jax_max_control_flow_unroll`: Public `trace()` option for the JAX control-flow unroll safety limit.
- `payload_policy`: Backend payload codec/materialization policy. JAX, tinygrad, MLX, and Paddle portable writes use `"array_payloads"` for non-torch array payloads, which is narrower than torch `"full"` payload portability; MLX and Paddle cover saved forward and derived arrays.
- JAX typed PRNG key payloads: Typed JAX PRNG keys round-trip through portable `.tlspec` saves as typed keys instead of being downgraded to legacy integer-key arrays.
- `PayloadLoadHints` / `JaxPayloadLoadHint`: Portable load hints exported from both `torchlens` and `torchlens.io`; JAX callers can pass an explicit sharding or request metadata reconstruction without changing `map_location`.
- `jax_sharding_*` codec metadata: `.tlspec` manifest fields for fully addressable JAX array payloads. They describe saved sharding provenance with JSON primitives such as sharding kind, mesh axis names, partition spec strings, and device counts; default load materializes values without reconstructing topology, while `PayloadLoadHints(jax=JaxPayloadLoadHint(...))` can opt into re-sharding.
- JAX multi-host sharded payloads: Multi-host or otherwise unaddressable JAX sharded arrays fail closed during portable save; fully addressable single-host sharded arrays are materialized by value with reconstructible sharding metadata.
- `save_preview`: Declared public `trace()` option for future non-torch save preview flags; the `save=` kwarg itself supports static-label selectors on JAX, tinygrad, MLX, and Paddle.
...
- `derived_grad`: JAX/tinygrad/MLX/Paddle derived intermediate gradient from `trace.intermediate_derived_grads`, when `GradOptions(intermediate_grads=True)` produced an exact oracle-confirmed match. This is not a saved true-backward gradient.
...
## Backend integration
- `Trace.backend`: Public `BackendName` tag for the capture backend (`"torch"`, `"mlx"`, `"jax"`, `"tinygrad"`, or `"paddle"` in shipped code). Survives portable save/load.
...
- `Trace.validation_replay_status`: `ValidationReplayStatus` machine-readable replay-validation status. Loaded JAX/tinygrad/Paddle traces whose runtime replay captures were stripped and loaded MLX traces without replay support report `state="unavailable"` and `reason="loaded_trace_runtime_capture_stripped"` instead of a pass/fail bool; live JAX/tinygrad/Paddle traces still run real replay validation, including static-label selectively saved traces via runtime-only hidden replay payloads. Importer-owned regions outside per-op replay report `state="unverified"` only after replayable checks pass; this status has `available=True` but is not a pass, and `bool(status)` raises.
...
- MLX capability contract: the MLX `BackendSpec` reports `validation_replay=False`, `backward_capture=False`, `fastlog=False`, `interventions=False`, `rng_replay=False`, `payload_materialization=True`, `intermediate_derived_grads=True`, `payload_policy="array_payloads"`, and `module_identity_modes=("function_root", "object_module")`. `mlx.nn.Module` roots use public `named_modules()` traversal for object-module hierarchy by default; `module_identity_mode="function_root"` keeps a root-only module surface. Static-label `save=` supports `tl.func`, `tl.label`, `tl.module`, `tl.in_module`, `tl.contains`, and boolean composites. `tl.backends.mlx.GradOptions` populates `trace.derived_grads` and, when `intermediate_grads=True`, exact custom-VJP-tap `trace.intermediate_derived_grads`. Portable saves round-trip saved forward and derived payloads as `mlx.core.array` values when the MLX runtime is installed; loaded MLX traces report replay validation unavailable rather than pass/fail. Unsupported public surfaces raise canonical backend errors where the registry owns dispatch; backend-private defensive checks may still raise existing concrete errors during the cutover.
- JAX capability contract: the JAX `BackendSpec` reports `validation_replay=True`, `backward_capture=False`, `fastlog=False`, `interventions=False`, `payload_materialization=True`, `intermediate_derived_grads=True`, `payload_policy="array_payloads"`, and `module_identity_modes=("function_root", "pytree_module")`. Raw callables use `function_root`; pure nested `jax.jit` calls inline through the concrete per-equation replay path, including JIT-wrapped control flow that then unrolls under the existing `jax_control_flow` policy. Over-cap `lax.scan`, over-cap `lax.while_loop`, and forward `custom_vjp_call` import as boundary/projection regions and report replay status `unverified` after replayable ops and region seams pass. Nested JIT calls with closed constants, effects, donated inputs, or explicit shardings remain rejected. Equinox and Flax NNX roots use `pytree_module` with strict rejection of inner JAX transforms/control flow and NNX state rebinding. Static-label `save=` selectors are post-finalization public-payload filters; value-dependent save predicates, `intervene=`, and `halt=` are rejected because jaxpr tracing does not expose public TorchLens labels plus concrete predicate-time mutation/replay semantics at one boundary. `tl.backends.jax.GradOptions` populates `trace.derived_grads` and, when `intermediate_grads=True`, exact oracle-confirmed `trace.intermediate_derived_grads`; true backward surfaces raise with guidance to derived-gradient accessors.
...
- Paddle capability contract: the Paddle `BackendSpec` reports `validation_replay=True`, `backward_capture=False`, `fastlog=False`, `interventions=False`, `payload_materialization=True`, `intermediate_derived_grads=True`, `payload_policy="array_payloads"`, and `module_identity_modes=("function_root", "object_module")`. Capture is dygraph/eager only. Static-label `save=` selectors are post-finalization public-payload filters; value-dependent predicates, `intervene=`, `halt=`, in-place mutation, RNG, tensor-derived Python scalar escapes, and active stochastic/training composites are rejected in the preview. Same-object no-ops are alias annotations. `tl.backends.paddle.GradOptions` populates `trace.derived_grads` and, when `intermediate_grads=True`, exact `trace.intermediate_derived_grads`. Portable saves round-trip saved forward and derived payloads as `paddle.Tensor` values when the Paddle runtime is installed; bf16 payloads store logical dtype metadata because NumPy transports them as `uint16`.
