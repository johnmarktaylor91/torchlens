...
- JAX multi-host sharded payloads: Multi-host or otherwise unaddressable JAX sharded arrays fail closed during portable save; only fully addressable single-host sharded arrays are materialized by value with audit-only sharding metadata.
...
- `derived_grad`: JAX/tinygrad derived intermediate gradient from `trace.intermediate_derived_grads`, when `GradOptions(intermediate_grads=True)` produced an exact oracle-confirmed match. This is not a saved true-backward gradient.
